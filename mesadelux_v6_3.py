#!/usr/bin/env python3
"""
MESADELUX v6.1 - Mesa de Luz Teatral
...
STRINGS = {
    'pt': {
        ...
        'patch.hint':           "DMX aceita vários endereços separados por '+'"
                                ...
                                "  · Alcunha / 8 bit / 16 bit → ascendente "
                                "(+1 por linha)   · Univ / Defeito → todas "
                                "com o mesmo valor.",
        ...
        'osc_help.text': (
            ...
            'N = número do canal / submaster (base 1).\n'
            '\n'
            '── RECEBE (consola → app) ──────────────────────────────\n'
            'Sem argumentos:\n'
            ...
            '  /pause               Pausa / Retoma\n'
            '  /blackout            Escuro (B.O.)\n'
            '  /clear               CLEAR — limpa a selecção\n'
            '  /soltar              RELEASE — liberta todos os canais\n'
            '  /rec/comprar         BACKUP — grava a memória\n'
            '  /rec/actualiza       UPDATE — actualiza a memória em curso\n'
            '  /rec/guarda          Guardar o show\n'
            '  /rec/cancelar        Cancelar a gravação\n'
            '\n'
            'Com argumentos:\n'
            '  /channel/{N}/level   i(0-255) ou f(<=1.0 normaliz.; >1.0=0-255)\n'
            '  /intensity           {canal:i}  {nivel:f}\n'
            '  /channel/select      {canal}\n'
            '  /submaster/{N}       f  (<=1.0 -> %;  >1.0 -> 0-100)\n'
            '  /group/{N}/level     f\n'
            '  /release_all         f  (fade ignorado — usa o tempo do menu)\n'
            '  /cue/go              GO — vai na cuelist\n'
            '  /cue/back            GOBACK — recua na cuelist\n'
            '  /cue/fade_in         f (segundos)\n'
            '  /cue/fade_out        f (segundos)\n'
            '  /cue/state/request   Pede o estado actual da cuelist\n'
            '\n'
            ...
            'Para configurar: Configurações -> OSC\n'
        ),
    },
    'en': {
        ...
        'patch.hint':           "DMX accepts several addresses separated by "
                                ...
                                "/ 8 bit / 16 bit → ascending (+1 per row)   "
        ...
        'osc_help.text': (
            ...
            'N = channel / submaster number (1-based).\n'
            '\n'
            '── RECEIVES (console → app) ────────────────────────────\n'
            'No arguments:\n'
            ...
            '  /soltar              RELEASE — release all channels\n'
            '  /rec/comprar         BACKUP — record cue\n'
            '  /rec/actualiza       UPDATE — update current cue\n'
            '  /rec/guarda          Save show\n'
            '  /rec/cancelar        Cancel recording\n'
            '\n'
            'With arguments:\n'
            '  /channel/{N}/level   i(0-255) or f(<=1.0 normalised; >1.0=0-255)\n'
            '  /intensity           {channel:i}  {level:f}\n'
            '  /channel/select      {channel}\n'
            '  /submaster/{N}       f  (<=1.0 -> %;  >1.0 -> 0-100)\n'
            '  /group/{N}/level     f\n'
            '  /release_all         f  (fade ignored — uses menu time)\n'
            '  /cue/go              GO — advance in cuelist\n'
            '  /cue/back            GOBACK — step back in cuelist\n'
            '  /cue/fade_in         f (seconds)\n'
            '  /cue/fade_out        f (seconds)\n'
            '  /cue/state/request   Request current cuelist state\n'
            '\n'
            '── SENDS (app → console, feedback) ─────────────────────\n'
...
def load_app_config():
    ...
    global LANG, MIDI_IN_PORT, MIDI_OUT_PORT
    try:
...
class Engine:
    ...
    def _tick(self):
        ...
        if self.fading and not self.paused:
            ...
            for ch in range(1, NUM_CHANNELS + 1):
                ...
                if tg >= s:                       # canal a subir → entrada
                    dur, delay = self.fade_in, self.delay_in
                else:                             # canal a descer → saída
                    dur, delay = self.fade_out, self.delay_out
                local = elapsed - delay
                if local <= 0:
                    t = 0.0
                elif dur > 0:
                ...
                if self.output[ch] != v:
                    self.output[ch] = v
                    changed = True
            if elapsed >= max(self.delay_in + self.fade_in,
    ...
    def _tick_fx(self, now):
        ...
        for i in range(len(self.fx)):
            ...
            if not rmp:
                continue
            k = (1.0 if rmp['dur'] <= 0.0
                 else min(1.0, (now - rmp['t0']) / rmp['dur']))
        ...
        with self._lock:
            ...
            for i in range(len(self.fx)):
                ...
                if desejado[i]:
                    ...
                    self.fx_active[i] = True
                    self._fx_run[i] = None
                    if snap or not com_fade[i] or f_in <= 0:
                        self._fx_scale[i] = 1.0
                        self._fx_ramp[i] = None
                    else:
                    ...
                    if snap or not com_fade[i] or f_out <= 0:
                        self.fx_active[i] = False
                        self._fx_run[i] = None
                        self._fx_scale[i] = 1.0
                        self._fx_ramp[i] = None
    ...
    def clear_programmer(self, duration=0.0):
    ...
    def release_output(self, pct_only, duration=0.0):
...
class PatchDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        for key, w in (('patch.col_channel', 6), ('patch.col_alias', 7),
            ...
            ttk.Label(hdr, text=T(key), width=w).pack(side=tk.LEFT, padx=1)
    ...
    def _bind_range(self, entry, col, ch):
        ...
        entry.bind('<Return>', lambda ev: self._range_fill(col))
    ...
    def _highlight_range(self):
        for ent in (list(self._alcunha_entries.values())
                    ...
                    + list(self._fine_entries.values())
...
class MesaDeLuxApp:
    def __init__(self, root):
        self.root = root
        self.root.title("MESADELUX v6.3")
    ...
    def _build_ui(self):
        ...
        dm.add_command(label=T('menu.clear_prog'),
                       command=self.engine.clear_programmer, accelerator="Esc")
        ...
        self.root.bind('<B>', lambda e: self._blackout())
        self.root.bind('<Escape>', lambda e: self.engine.clear_programmer())
    ...
    def _update_grid(self):
        ...
        for ch, (halo_id, rect_id, bar_id, name_id, num_id, val_id, x, y) \
            ...
            state = (lvl_i, prog, selected, passive, sub_dom, fx_dom, curva,
                     halo, name, alc, display, highlighted, moving)
    ...
    def _on_clear(self):
        """LIMPA: 1 toque limpa a SELECÇÃO; 2.º toque (até 1.2 s) limpa o
        PROGRAMADOR (azul), com fade no Tempo Liberta/Limpa."""
        now = time.time()
        if now - self._clear_last < self._MULTI_PRESS_S:
            self._capture_undo()                  # undo da limpeza do programador
    ...
    def _on_liberta(self):
        """LIBERTA: 1 toque deita abaixo as INTENSIDADES (0-100%) do output ao
        defeito; 2.º toque (até 1.2 s) deita TUDO (inclui atributos 0-255) ao
        defeito — sem selecção, sem programador, sem output. Ambos com fade no
        Tempo Liberta/Limpa. Para construir uma memória do zero."""
        now = time.time()
        full = (now - self._liberta_last < self._MULTI_PRESS_S)
        self._capture_undo()                      # undo da deitada-abaixo
        self.engine.release_output(pct_only=not full, duration=self._release_time)
        if full:
            self._selected = set()
            self._sel_locked = False
    ...
    def _build_cue_panel(self, parent):
        ...
        self._tree.column('fx',      width=44, anchor='center')
        ...
        if self._delete_armed:
            ...
            if levels:
                ...
                if self._selected:
                    ...
                    self.engine.recall_retrato(idx, only=set(self._selected))
                else:
    ...
    def _fx_link_txt(c):
        ...
        if not (1 <= n <= NUM_FX):
            return '·'
        return f"{n}∿" if ln.get('fade') else f"{n}⚡"
    ...
    def _refresh_cue_panel(self):
        ...
        nxt = self.engine.next_index(idx)
        if nxt is not None and 0 <= nxt < len(self.engine.cues):
            nc = self.engine.cues[nxt]
            fol = (self.engine.cues[idx].get('follow')
                   if 0 <= idx < len(self.engine.cues) else None)
            is_auto = fol is not None
    ...
    def _push_channel_alcunha(self, ch):
        ...
        self._send_console('/channel/{}/alcunha'.format(ch), alc)
    ...
    def _push_channel_curva(self, ch):
        ...
        self._send_console('/channel/{}/curva'.format(ch), str(curva))
    ...
    def _push_all_channel_names(self):
        ...
        for ch in range(1, NUM_CHANNELS + 1):
            ...
            msgs.append(('/channel/{}/alcunha'.format(ch), alc))
            msgs.append(('/channel/{}/curva'.format(ch),   curva))
        chans = [str(ch) for ch in range(1, NUM_CHANNELS + 1)
                 if self.engine.patch.get(ch).get('addrs')]
    ...
    def _osc_apply(self, addr, args):
        ...
        simple = {
            ...
            '/clear':         self.engine.clear_programmer,
