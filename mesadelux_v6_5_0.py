#!/usr/bin/env python3
"""
MESADELUX v6.5.0 - Mesa de Luz Teatral
Saída DMX: sACN (E1.31) + Art-Net (nativo, sem dependências) + USB-DMX
(adaptadores "Open DMX" FTDI) em paralelo.
Entrada: OSC (consolas físicas Pico W / bridge USB).
Requires: pip install sacn python-osc pyserial

v6.5.0 — Layouts, Etapa E1 (By Worm):
  · Vista de LAYOUT estilo GrandMA no painel esquerdo, como a grelha e
    o monitor DMX — tecla «LY» ao lado da «DMX» (CH ⇄ LY ⇄ DMX; também
    no menu Desk → Layout…). Barra de PÁGINAS no topo: um show pode ter
    vários layouts (p.ex. CONVENCIONAL / MÁQUINAS / LEDS). Células com
    nº/alcunha, nome, valor e halo + blocos de fundo com nome, a partir
    da secção OPCIONAL «layouts» do .ldsk. Actualização cirúrgica
    ~25 Hz, só células cujo estado mudou; a saída DMX nunca espera
    pelo desenho. Ficheiros sem a secção abrem como sempre; a v6.4.2
    ignora a secção ao abrir shows v6.5. Spec: MESADELUX_V6_5_LAYOUTS.md.
  · E2 — SELECÇÃO na vista: clique numa célula selecciona/desselecciona;
    arrastar faz laço tracejado ADITIVO; é a MESMA selecção central da
    app (grelha/expressão/consola seguem, e vice-versa); o modo P/T
    filtra como em todo o lado. Esc NÃO limpa (na app Esc = LIBERTA;
    limpar selecção = LIMPA 1×, coerente com a grelha).
  · E3a — EDIÇÃO por TAKE (lógica de sempre, dois toques): seleccionar
    canais → COMPRAR (arma) → LY → tecla da PÁGINA junta-os a essa
    página (células novas por baixo); tecla «+» cria página nova (pede
    nome). Barra de 8 TECLAS DE ATRIBUTO em baixo: COMPRAR armado +
    tecla = dar-lhe nome (ZOOM, RED, STROBE, PAN…); clique = toggle
    (uma activa de cada vez). Grava tudo no .ldsk (chave opcional
    layout_teclas) e entra no undo.
  · Teclas de atributo A FUNCIONAR (semântica = P/T generalizado,
    exclusivas com o P/T): ao ACTIVAR, a selecção expande pela alcunha
    e filtra aos canais cujo NOME começa pelo nome da tecla E são
    0-255 (display dec) — regra do autor — mostrando logo o valor; ao
    DESACTIVAR volta aos canais de intensidade (pct) das máquinas.
    APAGAR armado + tecla limpa o nome; APAGAR armado + tecla de
    página apaga a página (com confirmação).
  · E3b — EDIÇÃO GRÁFICA: botão EDIT na barra de páginas (vermelho
    quando activo; nunca junto com o comando — spec): arrastar células
    com ÍMAN de grelha (linhas de alinhamento visíveis), botão direito
    tira a célula da página (o canal fica no show); tudo com Ctrl+Z.
  · Falta: blocos↔grupos clicáveis (E4); geração automática a partir
    do patch (E6); renomear/reordenar páginas.

v6.4.2 — Selecção PAN/TILT por alcunha (By Worm):
  · Tecla «P/T» ao lado do H/S (grelha): cicla P/T → PAN → TILT → P/T.
    Com o modo activo, qualquer selecção da app (rectângulo, clique
    direito, expressão escrita, grupos, TODOS) expande pela ALCUNHA ao
    aparelho inteiro e filtra aos canais cujo NOME começa por PAN ou
    TILT (inclui PAN16/TILT16 — nomes do GDTF/«Repetir Aparelho»).
    Máquinas sem alcunha: filtra na mesma dentro da selecção. Carregar
    na tecla com uma selecção já feita transforma-a logo. A navegação
    </>, a consola e os retratos NÃO são filtrados. Consola 1 (tecla
    física + dois encoders pan/tilt) fica para uma versão futura.

v6.4.1 — Saída USB-DMX e gestão de universos (By Worm):
  · Etapa 1: os universos emitidos são EXACTAMENTE os do patch/
    renumerador — deixou de existir selecção manual de universos no
    menu (o modo manual da v4 foi removido; o patch é a fonte única
    de verdade). sACN e Art-Net emitem só nos universos patchados,
    recalculados ao carregar show e ao alterar o patch.
  · Etapas seguintes: módulo dmx_usb.py (adaptadores USB tipo "Open
    DMX", clones FTDI — DSD TECH SH-RS09B como hardware de referência),
    menu «Configuração DMX» com toggles independentes sACN/Art-Net/USB.
    Ver MESADELUX_V641_USBDMX_SPEC.md.
...
try:
    import mido
    # v6.4.2c — IMPORT EXPLÍCITO do backend rtmidi: o mido carrega-o
    # DINAMICAMENTE e o PyInstaller não o descobre sozinho — o exe
    # compilado ficava sem dispositivos MIDI (lista de portas vazia).
    # Com o import explícito o backend (e o python-rtmidi) entram no exe.
    import mido.backends.rtmidi          # noqa: F401 (p/ o PyInstaller)
    mido.set_backend('mido.backends.rtmidi')
    HAS_MIDI = True
except ImportError:
    HAS_MIDI = False

# v6.4.1 — USB-DMX (módulo isolado dmx_usb.py + pyserial). SEMPRE
# opcional: sem o módulo ou sem pyserial, a app arranca na mesma com o
# USB-DMX desactivado e aviso no terminal.
try:
    import dmx_usb
    HAS_DMX_USB = dmx_usb.PYSERIAL_OK
    if not HAS_DMX_USB:
        print('USB-DMX desactivado: pyserial não instalado '
              '(pip install pyserial).')
except Exception as _e:
    dmx_usb = None
    HAS_DMX_USB = False
    print('USB-DMX desactivado (módulo dmx_usb):', _e)
...
# v6.4.1 — SAÍDA DMX (preferência da máquina, não do show): toggle
# independente por protocolo (todos podem estar activos em simultâneo —
# os buffers de universo são a fonte única, cada protocolo é só um
# emissor) e o dispositivo USB-DMX configurado (UM único, por desenho:
# 1 dongle = 1 universo; vários universos resolvem-se com rede).
DMX_SACN_ACTIVO = True
DMX_ARTNET_ACTIVO = False
DMX_USB_ACTIVO = False
# {'id','tipo_id'('serial'|'porta'),'porta_com','universo','refresh_hz'}
# ou None quando nenhum dispositivo foi configurado.
DMX_USB_DISPOSITIVO = None
...
STRINGS = {
    'pt': {
        ...
        # (v6.4.2b: o patch.hint foi REMOVIDO da janela — roubava espaço
        # de trabalho, pedido do autor; a ajuda vive no manual)
        ...
        # v6.4.2 — inverter a saída DMX (típico p/ PAN/TILT ao contrário)
        'patch.col_invert':     'Inverte',
        'patch.inv_on':         'inverte',
        ...
        # v6.4.1 — os universos emitidos são os do patch, sem selecção manual
        'set.univ_patch':       'Universos emitidos: definidos pelo patch '
                                '({n} activo(s)): {u}',
        'set.unicast_ip':       'IP Unicast:',
        'set.if_multicast_off': '(usado se Multicast OFF)',
        'set.iface_note':       '(ETH00 = placa principal, vazio = auto; '
                                'ETH01 = 2ª placa OPCIONAL — emite pelas '
                                'duas em simultâneo, ex: ethernet + wifi)',
        'set.apply_sacn':       'APLICAR sACN',
        'set.an_enable':        'Activar Art-Net (paralelo ao sACN)',
        'set.an_univ':          'Universos (do patch): {u}\n'
        ...
        # ── vista DMX (v6.4.2: estava hard-coded em PT) ──
        'dmx.header':           'Saída DMX — Universo',
        'dmx.hint_r':           '  (R com 1 endereço selec. = renumerar '
                                'inverso)',
        ...
        # ── USB-DMX + menu Configuração (v6.4.1/b) ──
        'menu.config':          'Configuração',
        'menu.lang':            'Idioma…',
        'menu.cuelist':         'Cuelist…',
        # ── Layouts (v6.5) ──
        'menu.layout':          'Layout…',
        'layout.title':         'Layout',
        'layout.none':          ('Este show não tem layouts.\n\n'
                                 'Selecciona canais, prime COMPRAR (arma) '
                                 'e depois a tecla «+» aqui em cima para '
                                 'criares a primeira página.'),
        'lay.page_name':        'Nome da página:',
        'lay.key_name':         'Nome da tecla (ex.: ZOOM, RED):',
        'lay.sem_sel':          ('Selecciona primeiro os canais (na '
                                 'grelha ou no Layout).'),
        'lay.del_page':         'Apagar a página «{n}»?',
        'menu.config_dmx':      'USB-DMX…',
        'dmxcfg.title':         'USB-DMX',
        'dmxcfg.usb_hdr':       'Adaptador USB-DMX',
        'dmxcfg.usb_on':        'Activo',
        'dmxcfg.disp':          'Dispositivo',
        'dmxcfg.univ':          'Universo',
        'dmxcfg.refresh':       'Refresh (Hz)',
        'dmxcfg.estado':        'Estado:',
        'dmxcfg.actualizar':    'Actualizar',
        'dmxcfg.est_activo':    'activo',
        'dmxcfg.est_nligado':   'não ligado',
        'dmxcfg.est_erro':      'erro: {e}',
        'dmxcfg.est_off':       'desligado',
        'dmxcfg.sem_disp':      '(nenhum adaptador FTDI detectado)',
        'dmxcfg.nligado_suf':   ' — não ligado',
        'dmxcfg.univ_fora':     'O universo {u} não existe no patch — o '
                                'adaptador USB emite zeros até esse '
                                'universo ser patchado.',
        'dmxcfg.univ_err':      'Universo inválido (1-63999).',
        'dmxcfg.refresh_err':   'Refresh inválido (25-40 Hz).',
        'dmxcfg.sem_pyserial':  'USB-DMX indisponível: instala o pyserial '
                                '(pip install pyserial) e reinicia a '
                                'aplicação.',
        'dmxcfg.usb_start_err': 'Erro ao abrir o adaptador USB-DMX:\n{e}',
        'menu.quit':            'Sair',
        'menu.desk':            'Mesa',
        ...
        'btn.full':             'Máximo',
        'btn.zero':             'Zero',
        # v6.4.2 — era 'ESCURO': demasiado largo, empurrava o FX para
        # fora do ecrã (pedido do autor). B.O. é o jargão de teatro.
        'btn.bo':               'B.O.',
        'btn.release':          'LIBERTA',
        'btn.catch':            'AGARRA',
        'btn.drop':             'LARGA',
    ...
    'en': {
        ...
        # v6.4.2 — invert the DMX output (typical for upside-down PAN/TILT)
        'patch.col_invert':     'Invert',
        'patch.inv_on':         'invert',
        ...
        # v6.4.1 — emitted universes come from the patch, no manual selection
        'set.univ_patch':       'Emitted universes: defined by the patch '
                                '({n} active): {u}',
        'set.unicast_ip':       'Unicast IP:',
        'set.if_multicast_off': '(used if Multicast OFF)',
        'set.iface_note':       '(ETH00 = main adapter, empty = auto; '
                                'ETH01 = OPTIONAL 2nd adapter — sends out '
                                'of both at once, e.g. ethernet + wifi)',
        'set.apply_sacn':       'APPLY sACN',
        'set.an_enable':        'Enable Art-Net (parallel to sACN)',
        'set.an_univ':          'Universes (from the patch): {u}\n'
        ...
        'set.midi_status':      'IN: {in_}\nOUT: {out}',
        # ── DMX view (v6.4.2: was hard-coded in PT) ──
        'dmx.header':           'DMX Output — Universe',
        'dmx.hint_r':           '  (R with 1 address selected = inverse '
                                'repatch)',
        ...
        'din.no_release':       'DMX-In incoming: the programmer is not '
                                ...
                                'Settings → DMX-In).',
        # ── USB-DMX + Configuration menu (v6.4.1/b) ──
        ...
        'menu.layout':          'Layout…',
        'layout.title':         'Layout',
        'layout.none':          ('This show has no layouts.\n\n'
                                 'Select channels, press TAKE (arms) and '
                                 'then the «+» key above to create the '
                                 'first page.'),
        'lay.page_name':        'Page name:',
        'lay.key_name':         'Key name (e.g. ZOOM, RED):',
        'lay.sem_sel':          ('Select channels first (on the grid or '
                                 'in the Layout).'),
        'lay.del_page':         'Delete page "{n}"?',
        'menu.config_dmx':      'USB-DMX…',
        'dmxcfg.title':         'USB-DMX',
        'dmxcfg.usb_hdr':       'USB-DMX adapter',
        'dmxcfg.usb_on':        'Active',
        'dmxcfg.disp':          'Device',
        'dmxcfg.univ':          'Universe',
        'dmxcfg.refresh':       'Refresh (Hz)',
        'dmxcfg.estado':        'Status:',
        'dmxcfg.actualizar':    'Refresh list',
        'dmxcfg.est_activo':    'active',
        'dmxcfg.est_nligado':   'not connected',
        'dmxcfg.est_erro':      'error: {e}',
        'dmxcfg.est_off':       'off',
        'dmxcfg.sem_disp':      '(no FTDI adapter detected)',
        'dmxcfg.nligado_suf':   ' — not connected',
        'dmxcfg.univ_fora':     'Universe {u} is not in the patch — the '
                                'USB adapter sends zeros until that '
                                'universe is patched.',
        'dmxcfg.univ_err':      'Invalid universe (1-63999).',
        'dmxcfg.refresh_err':   'Invalid refresh (25-40 Hz).',
        'dmxcfg.sem_pyserial':  'USB-DMX unavailable: install pyserial '
                                '(pip install pyserial) and restart the '
                                'application.',
        'dmxcfg.usb_start_err': 'Error opening the USB-DMX adapter:\n{e}',
        'menu.quit':            'Quit',
        'menu.desk':            'Desk',
        'menu.patch':           'Patch…',
...
def load_app_config():
    ...
    global DMX_SACN_ACTIVO, DMX_ARTNET_ACTIVO, DMX_USB_ACTIVO
    global DMX_USB_DISPOSITIVO
    try:
        ...
        # v6.4.1 — toggles de saída + dispositivo USB-DMX
        if isinstance(cfg.get('dmx_sacn_activo'), bool):
            DMX_SACN_ACTIVO = cfg['dmx_sacn_activo']
        if isinstance(cfg.get('dmx_artnet_activo'), bool):
            DMX_ARTNET_ACTIVO = cfg['dmx_artnet_activo']
        if isinstance(cfg.get('dmx_usb_activo'), bool):
            DMX_USB_ACTIVO = cfg['dmx_usb_activo']
        dd = cfg.get('dmx_usb_dispositivo')
        if isinstance(dd, dict) and dd.get('porta_com'):
            DMX_USB_DISPOSITIVO = {
                'id': dd.get('id') or dd.get('porta_com'),
                'tipo_id': ('serial' if dd.get('tipo_id') == 'serial'
                            else 'porta'),
                'porta_com': dd.get('porta_com'),
                'universo': max(1, min(63999,
                                       int(dd.get('universo', 1) or 1))),
                'refresh_hz': max(25, min(40,
                                          int(dd.get('refresh_hz', 30)
                                              or 30))),
            }
    except Exception:
        pass
    ...
    try:
        with open(APP_CONFIG_PATH, 'w', encoding='utf-8') as f:
            json.dump({'lang': LANG,
                       ...
                       # v6.4.1 — saída DMX (toggles + dispositivo USB)
                       'dmx_sacn_activo': DMX_SACN_ACTIVO,
                       'dmx_artnet_activo': DMX_ARTNET_ACTIVO,
                       'dmx_usb_activo': DMX_USB_ACTIVO,
                       'dmx_usb_dispositivo': DMX_USB_DISPOSITIVO},
...
# ─────────────────────────────────────────────
# v6.4.2 — emissão por VÁRIAS placas de rede em simultâneo (pedido do
# autor: ethernet + wifi com o MESMO protocolo). O campo «Interface»
# aceita vários IPs separados por vírgula; cria-se um emissor por placa
# e estes wrappers fazem fan-out mantendo a interface que o motor usa.
# ─────────────────────────────────────────────
class _MultiOutSACN:
    """Proxy de UM universo espalhado por vários senders sACN."""
    ...
    def destination(self):
        return getattr(self._outs[0], 'destination', None)
    ...
    @property
    def multicast(self):
        return getattr(self._outs[0], 'multicast', None)
    ...
    @property
    def dmx_data(self):
        return getattr(self._outs[0], 'dmx_data', None)
...
class MultiSACN:
    """Fan-out para 1..N senders sACN (um por interface de rede).
    Espelha a superfície da lib sacn que o motor usa: start/stop,
    activate/deactivate_output, get_active_outputs e sender[u]."""

    def __init__(self, binds, source_name):
        self._senders = [sacn.sACNsender(bind_address=b,
    ...
    def get_active_outputs(self):
        return (self._senders[0].get_active_outputs()
                if self._senders else [])

    def __getitem__(self, u):
        outs = []
        for s in self._senders:
            o = s[u]
            if o is None:       # universo ainda não activo neste sender
                return None
            outs.append(o)
        return _MultiOutSACN(outs) if outs else None


class MultiArtNet:
    """Fan-out para 1..N ArtNetSender (um por interface de rede)."""

    def __init__(self, bind_ips, dest_ip, broadcast):
        self._senders = [ArtNetSender(bind_ip=b, dest_ip=dest_ip,
                                      broadcast=broadcast)
                         for b in bind_ips]

    def send(self, universe, data512):
        for s in self._senders:
            s.send(universe, data512)
...
def parse_bind_ips(texto, vazio):
    """'ip1, ip2' → lista de IPs de bind; vazio/None → [vazio]."""
    binds = [b.strip() for b in str(texto or '').split(',') if b.strip()]
    return binds or [vazio]
...
def export_ascii(show_dict, filepath, follow_estilo='eos'):
    ...
    L = ['IDENT 3:0',
         'MANUFACTURER ByWorm',
         'CONSOLE MesaDeLux v6.4.2',
         'CLEAR ALL',
         '! Exportado pela MesaDeLux v6.4.2 (By Worm)',
...
def normaliza_layouts(raw):
    """v6.5 — valida/limpa a lista 'layouts' vinda do ficheiro .ldsk.
    Devolve SEMPRE uma lista (vazia se nada aproveitável) — a ausência
    de layouts é caso perfeitamente normal, nunca erro; dados doentes
    não impedem a app de arrancar (princípio 3 da spec). Uma célula que
    aponte para um canal fora do show é mantida (a vista desenha-a a
    cinzento). Posições em UNIDADES DE GRELHA, >= 0."""
    ...
    for i, l in enumerate(raw):
        ...
        for c in (l.get('celulas') or []):
            ...
            try:
                ch = int(c.get('canal'))
                col = max(0, int(c.get('coluna', 0)))
                lin = max(0, int(c.get('linha', 0)))
            except (TypeError, ValueError):
                continue
            if ch < 1 or ch in vistos:      # um canal só 1x por layout
                continue
            vistos.add(ch)
            cels.append({'canal': ch, 'coluna': col, 'linha': lin})
        blocos = []
        for b in (l.get('blocos') or []):
            if not isinstance(b, dict):
                continue
            bch = []
            for ch in (b.get('celulas') or []):
                try:
                    ch = int(ch)
                except (TypeError, ValueError):
                    continue
            ...
            try:
                ...
                gid = None
            if gid is not None and not (0 <= gid < 20):
                gid = None
            if bch:
                blocos.append({'nome': str(b.get('nome', '') or '')[:20],
                               'grupo_id': gid, 'celulas': sorted(bch)})
        if not cels:
            continue
        try:
            lid = int(l.get('id', i + 1))
        except (TypeError, ValueError):
            lid = i + 1
        out.append({'id': lid,
                    'nome': str(l.get('nome', '') or f'LAYOUT {lid}')[:20],
                    'celulas': cels, 'blocos': blocos})
    return out
...
class Engine:
    def __init__(self):
        ...
        self.layouts = []                 # v6.5: páginas de layout (no show)
        self.layout_teclas = [''] * 10    # v6.5: teclas de atributo (nomes;
                                          # 8→10 a pedido do autor 2026-07-21)
        ...
        # v6.4.1: os universos de saída derivam SEMPRE do renumerador
        # ("o que se patcha é o que sai") — ver out_universes(). O modo
        # manual da v4 (lista sacn_universes + universes_auto) foi
        # removido: o patch é a fonte única de verdade.
        ...
        # v6.4.1 — USB-DMX ("Open DMX", clones FTDI) em paralelo com os
        # protocolos de rede. UM dispositivo, UM universo atribuído.
        self.usb_tx = None            # TransmissorDMXUSB activo (ou None)
    ...
    # ── DMX / sACN ────────────────────────────
    # (v6.4.1: _norm_universes foi removida com o modo manual — os
    # universos de saída derivam sempre do patch, ver out_universes)
    ...
    def out_universes(self):
        """Lista EFECTIVA de universos de saída (comum a sACN, Art-Net e
        USB-DMX).

        v6.4.1: derivada SEMPRE do renumerador — o que se patcha é o que
        sai, sem excepções nem configuração. Sem patch nenhum, emite U1
        (heartbeat)."""
    ...
    def _flush_dmx(self):
        ...
        for ch in range(1, NUM_CHANNELS + 1):
            ...
            if e.get('bit16'):
                ...
                # v6.4.2 — INVERTER DMX (típico p/ PAN/TILT montados ao
                # contrário): inverte o valor de 16 bits INTEIRO — cobre
                # os dois endereços (coarse+fine) de uma vez
                if e.get('invert'):
                    val16 = 65535 - val16
                ...
                # v6.4.2 — INVERTER DMX: aplica-se DEPOIS da curva (é uma
                # transformação do fio, não do valor da mesa). A curva
                # «ligado» NÃO inverte — ligado é sempre ligado (correcção
                # do autor 2026-07-18: invertê-la dava um canal sempre a 0).
                if e.get('invert') and curva != CURVE_LIGADO:
                    v = 255 - v
                for a in addrs:
                    if 1 <= a <= 512:
                        buf[a - 1] = v
    ...
    def start_sacn(self, multicast=True,
                   unicast_ip="192.168.1.10", bind_ip=""):
        """Activa a saída sACN nos universos do patch (v6.4.1: a lista
        vem sempre do renumerador — ver out_universes()):
        ...
        try:
            ...
            # v6.4.2 — várias placas: "ip1, ip2" cria um sender por IP
            binds = parse_bind_ips(self.sacn_bind_ip, '0.0.0.0')
            bind = ', '.join(binds)
            # lista efectiva — vem do renumerador
            univs = list(self.out_universes())
            print(f'[sACN] start: multicast={self.sacn_multicast} '
                  f'unicast_ip="{self.sacn_unicast_ip}" bind="{bind}" '
                  f'universos={univs}')

            # 3) Cria sender(s) novo(s) e arranca
            self.sacn_sender = MultiSACN(binds, SACN_SOURCE_NAME)
            ...
            msg = (f"sACN activo — {len(univs)} universo(s) do patch: {lst}\n"
                   f"Modo: {mode}\nBind: {bind}")
            if len(univs) > 8 and self.sacn_multicast:
    ...
    def start_artnet(self, broadcast=True, dest_ip='', bind_ip=''):
        ...
        ligado). Usa a MESMA lista de universos de saída (do patch —
        out_universes()). Devolve (ok, mensagem)."""
        try:
            with self._lock:
                ...
                # v6.4.2 — várias placas: "ip1, ip2" cria um socket por IP
                self.artnet_sender = MultiArtNet(
                    parse_bind_ips(self.artnet_bind_ip, ''),
                    self.artnet_dest_ip,
            ...
            print('[Art-Net] start: {} bind={} universos={}'.format(
                mode, bind, univs))
            msg = ("Art-Net activo — {} universo(s) do patch: {}\n"
                   "Modo: {}\nBind: {}\n"
                   "(universo da mesa N = Art-Net N-1)"
                   .format(len(univs), lst, mode, bind))
            if len(univs) > 8 and self.artnet_broadcast:
    ...
    # ── USB-DMX (v6.4.1) ──────────────────────
    def _usb_obter_buffer(self):
        """Cópia thread-safe do buffer do universo atribuído ao dongle.
        Universo fora do patch → não há buffer → zeros (o dongle emite
        na mesma, conforme a spec: não se bloqueia o utilizador que
        ainda vai patchar)."""
        with self._lock:
            buf = self.universes.get(self.usb_universo)
            return bytes(buf) if buf is not None else bytes(512)

    def start_usb_dmx(self, porta_com, universo, refresh_hz):
        """Arranca (ou re-arranca) a transmissão USB-DMX na porta dada.
        Devolve (ok, msg_erro)."""
        if not HAS_DMX_USB:
            return False, 'pyserial/dmx_usb indisponível'
        self.stop_usb_dmx()
        self.usb_universo = max(1, min(63999, int(universo)))
        self.usb_refresh = int(refresh_hz)
        driver = dmx_usb.DriverOpenDMX(porta_com)
        tx = dmx_usb.TransmissorDMXUSB(driver, self._usb_obter_buffer,
                                       refresh_hz=self.usb_refresh)
        ok, err = tx.arrancar()
        if not ok:
            return False, err
        self.usb_tx = tx
        self.usb_enabled = True
        print('[USB-DMX] start: %s universo=%d refresh=%dHz'
              % (porta_com, self.usb_universo, tx.refresh_hz))
        return True, None

    def stop_usb_dmx(self):
        if self.usb_tx is not None:
            try:
                self.usb_tx.parar()
            except Exception:
                pass
    ...
    def usb_estado(self):
        """Estado da transmissão USB: ('inactivo'|'activo'|'erro', erro).
        'erro' fica visível (dongle desligado a quente) até novo arranque."""
        if self.usb_tx is None:
            return 'inactivo', None
        return self.usb_tx.estado, self.usb_tx.erro
    ...
    def clear_cues(self):
        ...
        self.groups = [None] * 20
        self.layouts = []                         # v6.5
        self.layout_teclas = [''] * 10            # v6.5 (10 teclas)
    ...
    def set_channel(self, ch, level, fino=False):
        """v6.4.2: `fino=True` guarda o nível como FLOAT (sem truncar) —
        para os passos fraccionários (±0,5/±2,5) dos encoders PAN/TILT
        da consola; nos canais 16 bit o fine deriva do float no flush.
        Sem `fino` mantém o comportamento de sempre (inteiro)."""
        if 1 <= ch <= NUM_CHANNELS:
            ...
            if fino:
                self.programmer[ch] = max(0.0, min(255.0, float(level)))
            else:
    ...
    def clear_programmer(self, duration=0.0, exclude=None):
        ...
        if duration and duration > 0:
            with self._lock:
                ...
                if prog_chs:
                    self.fade_start = self.output[:]
                    # v6.4.2 FIX: SEM transição em curso, os canais que NÃO
                    # se libertam têm de ficar PARADOS (alvo = valor actual).
                    # Sem isto herdavam o fade_target VELHO da última
                    # transição real — e como o SEQ± em cue-only aplica as
                    # memórias A SECO (mexe no output, não nos alvos), um
                    # LIMPA/LARGA depois de navegar punha canais alheios a
                    # viajar para valores antigos (visto com CATCH/DROP:
                    # «cores sobem ao máximo sem estarem na memória»).
                    # Com transição em curso mantém-se o comportamento de
                    # 2026-06-23 (não reescrever, senão congelava a meio).
                    if not self.fading:
    ...
    def _show_data(self):
        ...
        d = {
            ...
            # v4: grava a lista EFECTIVA (derivada do renumerador) — assim
            # a v3 abre o show e emite o mesmo.
            ...
            # v6.4.1: deixámos de gravar universes_auto/manual_universes —
            # o patch é a fonte única; versões antigas assumem auto=True.
            'output_cfg': {
        ...
        # v6.5 — layouts: secção OPCIONAL, só escrita quando existe.
        # Sem layouts o ficheiro fica IGUAL ao da v6.4.2; a v6.4.2
        # ignora a chave ao abrir shows v6.5 (lê chave a chave).
        if self.layouts:
            d['layouts'] = self.layouts
        if any(self.layout_teclas):
            d['layout_teclas'] = self.layout_teclas
        return d
    ...
    def _load_data(self, data):
        ...
        # v6.5 — layouts (secção opcional; shows antigos não a têm e é
        # caso normal; dados doentes → lista vazia, nunca erro)
        self.layouts = normaliza_layouts(data.get('layouts'))
        lt = data.get('layout_teclas', [])
        if not isinstance(lt, list):
            lt = []
        self.layout_teclas = [
            (str(lt[i]).strip().upper()[:8]
             if i < len(lt) and lt[i] else '')
            for i in range(10)]     # 10 teclas (shows antigos: 8 + 2 vazias)
        ...
        self._fx_scale = [1.0] * NUM_FX
        self._fx_ramp = [None] * NUM_FX
        # v6.4.1: os campos 'sacn_universes'/'sacn_universe' de shows
        # antigos são IGNORADOS — os universos de saída derivam sempre
        # do patch acabado de carregar (out_universes()).
        # v4: restaura a configuração de saída (sACN modo + Art-Net) se o
        # show a tiver gravado. Shows v3 não têm este campo — mantêm
        # defaults. universes_auto/manual_universes antigos são ignorados.
        ...
        if isinstance(oc, dict):
            ...
            # v6.4.1: 'artnet_enabled' do show é IGNORADO — ligar/desligar
            # cada protocolo é preferência da máquina (Configuração DMX),
            # não do show. Os parâmetros (broadcast/IP) continuam a viajar.
...
class PatchDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        # (v6.4.2b: o texto de ajuda saiu — roubava espaço de trabalho)
        hdr = ttk.Frame(top)
        hdr.pack(fill=tk.X)
    ...
    def _build_rows(self):
        ...
        for ch in range(1, NUM_CHANNELS + 1):
            ...
            ttk.Combobox(row, textvariable=cu,
                         ...
                         width=8, state='readonly').pack(side=tk.LEFT, padx=1)
            # v6.4.2 — INVERTER DMX (saída no fio: v -> 255-v; nos 16 bit
            # inverte o valor inteiro, cobre coarse+fine). Típico p/
            # PAN/TILT de máquinas montadas ao contrário. v6.4.2b: menu
            # (vazio | inverte) em vez de checkbox, pedido do autor.
            iv = tk.StringVar(value=(T('patch.inv_on')
                                     if e.get('invert') else ''))
            ttk.Combobox(row, textvariable=iv,
                         values=['', T('patch.inv_on')],
                         width=7, state='readonly').pack(side=tk.LEFT,
                                                         padx=1)
            self._rows[ch] = (av, nv, uv, cv, fv, gv, hv, dv, cu, iv)
        ...
        for i in range(r['qtd']):
            ...
            for name, is16, dflt in foot:
                ...
                if ch > NUM_CHANNELS:
                    ...
                    # v6.4.2: o tuplo ganhou o iv (coluna Inverte) — sem
                    # ele aqui o unpack rebentava e o carimbo morria
                    (av, nv, uv, cv, fv, gv, hv, dv, cu,
                     iv) = self._rows[ch]
                    up = name.strip().upper()
    ...
    def _apply(self):
        dropped = 0
        for ch, (av, nv, uv, cv, fv, gv, hv, dv, cu,
                 iv) in self._rows.items():
            try:
                u = int(uv.get())
            except ValueError:
                u = 1
...
class SettingsDialog(tk.Toplevel):
    """v6.4.1b — cada secção de configuração abre como janela PRÓPRIA:
    SettingsDialog(parent, app, only='sacn'|'artnet'|'osc'|'midi'|
    'lang'|'cuelist'|'dmxin') constrói só essa secção (entradas do menu
    «Configuração»; Idioma e Cuelist no menu Mesa). Sem `only`, monta o
    antigo bloco de separadores completo (sem entrada de menu — recurso)."""

    def __init__(self, parent, app, only=None):
        super().__init__(parent)
        self.app = app
        self.only = only
        if only is None:
    ...
    def _build(self):
        defs = (('sacn',    ' sACN ',               self._tab_sacn),
                ('artnet',  ' Art-Net ',            self._tab_artnet),
                ('osc',     ' OSC ',                self._tab_osc),
                ('midi',    T('settings.midi_tab'), self._tab_midi),
                ('lang',    T('settings.lang_tab'), self._tab_lang),
                ('cuelist', T('settings.cl_tab'),   self._tab_cuelist),
                ('dmxin',   T('settings.din_tab'),  self._tab_dmxin))

        if self.only:
            # janela individual: só a secção pedida (cada secção tem o
            # seu próprio botão APLICAR) + Fechar
            for key, label, fn in defs:
                if key == self.only:
                    self.title(label.strip())
                    fn(self).pack(fill=tk.BOTH, expand=True,
                                  padx=6, pady=6)
                    break
    ...
    # ── sACN ──────────────────────────────────
    def _tab_sacn(self, parent):
        sf = ttk.Frame(parent, padding=12)

        # v6.4.1b — o toggle Activar voltou para aqui (persiste como
        # preferência da máquina, DMX_SACN_ACTIVO)
        ...
        # v6.4.1 — sem selecção manual: os universos emitidos são
        # exactamente os do patch/renumerador, sem excepções.
        outs = self.app.engine.out_universes()
        agora = ', '.join('U' + str(u) for u in outs)
        ttk.Label(sf, text=T('set.univ_patch', n=len(outs), u=agora),
        ...
        # v6.4.2b — DUAS interfaces de saída (pedido do autor): ETH00 é a
        # principal (vazio = auto) e ETH01 a segunda placa OPCIONAL
        # (ethernet + wifi em simultâneo, o mesmo protocolo nas duas)
        binds = parse_bind_ips(self.app.engine.sacn_bind_ip, '')
        ips = local_ips()
        ttk.Label(sf, text='ETH00:').grid(row=9, column=0, sticky='w')
        self._bind_ip = tk.StringVar(value=binds[0])
        ttk.Combobox(sf, textvariable=self._bind_ip, width=16,
                     values=[""] + ips).grid(row=9, column=1, sticky='w')
        ttk.Label(sf, text='ETH01:').grid(row=10, column=0, sticky='w')
        self._bind_ip2 = tk.StringVar(value=binds[1] if len(binds) > 1
                                      else '')
        ttk.Combobox(sf, textvariable=self._bind_ip2, width=16,
                     values=[''] + ips + ['0.0.0.0', '127.0.0.1']).grid(
            row=10, column=1, sticky='w')
        ttk.Label(sf, text=T('set.iface_note'),
                  foreground='gray', wraplength=400, justify='left').grid(
        ...
        ttk.Separator(sf, orient='horizontal').grid(
            row=12, column=0, columnspan=2, sticky='ew', pady=(8, 4))
        sb = tk.Button(sf, text=T('set.apply_sacn'), command=self._apply_sacn,
                       bg='#2c7d4f', fg='white', font=('Arial', 10, 'bold'),
                       relief=tk.FLAT, padx=12, pady=6, cursor='hand2')
        sb.grid(row=13, column=0, columnspan=2, pady=(2, 2))
        return sf

    # ── Art-Net (v4) ──────────────────────────
    def _tab_artnet(self, parent):
        af = ttk.Frame(parent, padding=12)

        eng = self.app.engine
        # v6.4.1b — o toggle Activar voltou para aqui (persiste como
        # preferência da máquina, DMX_ARTNET_ACTIVO)
        ...
        ttk.Label(af, text=T('set.an_univ', u=univ_txt),
        ...
        # v6.4.2b — duas interfaces (ETH00 principal + ETH01 opcional)
        an_binds = parse_bind_ips(eng.artnet_bind_ip, '')
        an_ips = local_ips()
        ttk.Label(af, text='ETH00:').grid(row=6, column=0, sticky='w')
        self._an_bind = tk.StringVar(value=an_binds[0])
        ttk.Combobox(af, textvariable=self._an_bind, width=16,
                     values=[""] + an_ips).grid(row=6, column=1, sticky='w')
        ttk.Label(af, text='ETH01:').grid(row=7, column=0, sticky='w')
        self._an_bind2 = tk.StringVar(value=an_binds[1]
                                      if len(an_binds) > 1 else '')
        ttk.Combobox(af, textvariable=self._an_bind2, width=16,
                     values=[''] + an_ips + ['0.0.0.0', '127.0.0.1']).grid(
            row=7, column=1, sticky='w')
        ttk.Label(af, text=T('set.iface_note'),
                  foreground='gray', wraplength=400,
                  justify='left').grid(row=8, column=0, columnspan=2,
                                       sticky='w')
        ...
        ab = tk.Button(af, text=T('set.apply_artnet'), command=self._apply_artnet,
                       bg='#2c7d4f', fg='white', font=('Arial', 10, 'bold'),
                       relief=tk.FLAT, padx=12, pady=6, cursor='hand2')
        ab.grid(row=10, column=0, columnspan=2, pady=(2, 2))
        return af

    # ── OSC ───────────────────────────────────
    def _tab_osc(self, parent):
        of = ttk.Frame(parent, padding=12)
    ...
    # ── MIDI (v6.3) ───────────────────────────
    def _tab_midi(self, parent):
        mf = ttk.Frame(parent, padding=12)
        if not HAS_MIDI:
    ...
    # ── Idioma / Language (v6.2 i18n) ─────────
    def _tab_lang(self, parent):
        lf = ttk.Frame(parent, padding=12)
    ...
    # ── Cuelist (v6.2: intensidade cue-only vs tracking) ──
    def _tab_cuelist(self, parent):
        clf = ttk.Frame(parent, padding=12)
    ...
    # ── DMX-In (v6.4, Etapa 1: só escuta) ─────
    def _tab_dmxin(self, parent):
        df = ttk.Frame(parent, padding=12)
    ...
    def _apply_sacn(self):
        # v6.4.1: os universos vêm sempre do patch (out_universes).
        # v6.4.1b: o toggle Activar voltou para aqui e persiste como
        # preferência da máquina (governa também o auto-arranque).
        global DMX_SACN_ACTIVO
        multicast = bool(self._multicast.get())
        ip = self._ip.get().strip()
        # v6.4.2b — junta ETH00 + ETH01 (segunda placa opcional)
        bind_ip = ', '.join(p.strip() for p in (self._bind_ip.get(),
                                                self._bind_ip2.get())
                            if p.strip())
        ...
        DMX_SACN_ACTIVO = bool(self._sacn_on.get())
        save_app_config()

        if self._sacn_on.get():
            ok, msg = self.app.engine.start_sacn(multicast, ip, bind_ip)
            if ok:
    ...
    def _apply_artnet(self):
        # v6.4.1b: o toggle Activar voltou para aqui e persiste como
        # preferência da máquina (governa também o auto-arranque).
        global DMX_ARTNET_ACTIVO
        eng = self.app.engine
        bcast = bool(self._an_bcast.get())
        ip = self._an_ip.get().strip()
        # v6.4.2b — junta ETH00 + ETH01 (segunda placa opcional)
        bind_ip = ', '.join(p.strip() for p in (self._an_bind.get(),
                                                self._an_bind2.get())
                            if p.strip())

        if self._an_on.get() and not bcast and not ip:
        ...
        DMX_ARTNET_ACTIVO = bool(self._an_on.get())
        save_app_config()
...
class ConfigDMXDialog(tk.Toplevel):
    """v6.4.1b — «USB-DMX»: configuração do adaptador USB (dispositivo
    único, universo atribuído, refresh) e o seu toggle Activo. Os
    toggles sACN/Art-Net vivem nas respectivas janelas do menu
    «Configuração». OK aplica, persiste na config da máquina e
    (re)arranca a thread de transmissão sem reiniciar a aplicação;
    Cancelar não mexe em nada."""

    def __init__(self, app):
        super().__init__(app.root)
        self.app = app
        self.title(T('dmxcfg.title'))
        ...
        # ── adaptador USB-DMX ──
        ttk.Label(frm, text=T('dmxcfg.usb_hdr'),
                  font=('Arial', 9, 'bold')).grid(
            row=5, column=0, columnspan=2, sticky='w', pady=(0, 4))
        self._usb_var = tk.BooleanVar(value=DMX_USB_ACTIVO)
        self._usb_chk = ttk.Checkbutton(frm, text=T('dmxcfg.usb_on'),
                                        variable=self._usb_var)
        self._usb_chk.grid(row=5, column=2, sticky='e', pady=(0, 4))

        if not HAS_DMX_USB:
            ttk.Label(frm, text=T('dmxcfg.sem_pyserial'),
                      foreground='#e67e22', wraplength=400,
                      justify='left').grid(
                row=6, column=0, columnspan=3, sticky='w',
                padx=(12, 0), pady=(0, 6))
            self._usb_chk.config(state='disabled')
            self._usb_var.set(False)

        cfg = DMX_USB_DISPOSITIVO or {}
        ttk.Label(frm, text=T('dmxcfg.disp')).grid(
            row=7, column=0, sticky='w', padx=(12, 6))
        self._disp_cb = ttk.Combobox(frm, width=34, state='readonly')
        self._disp_cb.grid(row=7, column=1, columnspan=2, sticky='w')

        ttk.Label(frm, text=T('dmxcfg.univ')).grid(
            row=8, column=0, sticky='w', padx=(12, 6), pady=(4, 0))
        self._univ_var = tk.StringVar(value=str(cfg.get('universo', 1)))
        ttk.Entry(frm, textvariable=self._univ_var, width=7).grid(
            row=8, column=1, sticky='w', pady=(4, 0))

        ttk.Label(frm, text=T('dmxcfg.refresh')).grid(
            row=9, column=0, sticky='w', padx=(12, 6), pady=(4, 0))
        self._hz_var = tk.StringVar(value=str(cfg.get('refresh_hz', 30)))
        tk.Spinbox(frm, textvariable=self._hz_var, width=5,
                   from_=dmx_usb.REFRESH_MIN if HAS_DMX_USB else 25,
                   to=dmx_usb.REFRESH_MAX if HAS_DMX_USB else 40,
                   bg='#1a1a2e', fg='#ccccee',
                   buttonbackground='#2c2c44').grid(
            row=9, column=1, sticky='w', pady=(4, 0))

        # linha de estado + Actualizar
        est_row = ttk.Frame(frm)
        est_row.grid(row=10, column=0, columnspan=3, sticky='ew',
                     padx=(12, 0), pady=(8, 2))
        ttk.Label(est_row, text=T('dmxcfg.estado')).pack(side=tk.LEFT)
        self._estado_lbl = tk.Label(est_row, text='', bg='#111122',
                                    fg='#555577',
                                    font=('Arial', 9, 'bold'))
        self._estado_lbl.pack(side=tk.LEFT, padx=(6, 0))
        ttk.Button(est_row, text=T('dmxcfg.actualizar'),
                   command=self._refresh_disps).pack(side=tk.RIGHT,
                                                     padx=(12, 0))
        ...
        btns = ttk.Frame(frm)
        btns.grid(row=12, column=0, columnspan=3)
        tk.Button(btns, text=T('common.ok'), command=self._ok,
                  bg='#2c7d4f', fg='white', font=('Arial', 10, 'bold'),
                  relief=tk.FLAT, padx=18, pady=6,
                  cursor='hand2').pack(side=tk.LEFT, padx=6)
        ttk.Button(btns, text=T('common.cancel'),
                   command=self.destroy).pack(side=tk.LEFT, padx=6)
    ...
    # ── detecção / dropdown ──
    def _refresh_disps(self):
        """(Re)detecta os adaptadores FTDI ligados e refaz o dropdown.
        Se o dispositivo configurado não estiver presente, aparece na
        lista com o sufixo «não ligado» (seleccionável — a config
        mantém-se para quando ele voltar)."""
        self._disps = (dmx_usb.listar_dispositivos()
                       if HAS_DMX_USB else [])
        vals = [d.rotulo() for d in self._disps]
        cfg = DMX_USB_DISPOSITIVO
        sel = None
        if cfg:
            for i, d in enumerate(self._disps):
                if ((cfg.get('tipo_id') == 'serial'
                     and d.serie == cfg.get('id'))
                        or (cfg.get('tipo_id') == 'porta'
                            and d.porta == cfg.get('porta_com'))):
                    sel = i
                    break
            if sel is None:
                vals.append('%s%s' % (cfg.get('porta_com')
                                      or cfg.get('id'),
                                      T('dmxcfg.nligado_suf')))
                sel = len(vals) - 1
        elif self._disps:
            sel = 0
        if not vals:
            vals = [T('dmxcfg.sem_disp')]
            sel = 0
        self._disp_cb['values'] = vals
        self._disp_cb.current(sel)
        self._update_estado()

    def _disp_escolhido(self):
        """DispositivoFTDI seleccionado no dropdown, ou None (entrada
        «não ligado» / nenhum adaptador)."""
        idx = self._disp_cb.current()
        if 0 <= idx < len(self._disps):
            return self._disps[idx]
        return None

    # ── linha de estado ──
    def _update_estado(self):
        est, erro = self.app.engine.usb_estado()
        if est == 'activo':
            txt, cor = '● ' + T('dmxcfg.est_activo'), '#2ecc71'
        elif est == 'erro':
            txt = '● ' + T('dmxcfg.est_erro', e=(erro or '?'))
            cor = '#e74c3c'
        elif self._usb_var.get() and self._disp_escolhido() is None:
            txt, cor = '● ' + T('dmxcfg.est_nligado'), '#8888aa'
        else:
            txt, cor = '● ' + T('dmxcfg.est_off'), '#555577'
        self._estado_lbl.config(text=txt, fg=cor)

    def _tick_estado(self):
        """Actualiza a linha de estado enquanto o diálogo está aberto
        (apanha o dongle a cair a meio)."""
        if not self.winfo_exists():
            return
        self._update_estado()
        self.after(500, self._tick_estado)

    # ── OK ──
    def _ok(self):
        global DMX_USB_ACTIVO, DMX_USB_DISPOSITIVO
        app, eng = self.app, self.app.engine

        # validação dos campos USB
        try:
            univ = int(self._univ_var.get().strip())
            if not 1 <= univ <= 63999:
                raise ValueError
        except (TypeError, ValueError):
            messagebox.showerror(T('dmxcfg.title'), T('dmxcfg.univ_err'),
                                 parent=self)
            return
        try:
            hz = int(self._hz_var.get().strip())
            if not 25 <= hz <= 40:
                raise ValueError
        except (TypeError, ValueError):
            messagebox.showerror(T('dmxcfg.title'), T('dmxcfg.refresh_err'),
                                 parent=self)
            return

        usb_on = bool(self._usb_var.get()) and HAS_DMX_USB

        # dispositivo: escolhido no dropdown, ou mantém o configurado
        # (entrada «não ligado»)
        disp = self._disp_escolhido()
        if disp is not None:
            DMX_USB_DISPOSITIVO = {
                'id': disp.serie or disp.porta,
                'tipo_id': 'serial' if disp.serie else 'porta',
                'porta_com': disp.porta,
                'universo': univ,
                'refresh_hz': hz,
            }
        elif DMX_USB_DISPOSITIVO:
            DMX_USB_DISPOSITIVO = dict(DMX_USB_DISPOSITIVO,
                                       universo=univ, refresh_hz=hz)

        DMX_USB_ACTIVO = bool(self._usb_var.get())
        save_app_config()

        # USB: pára sempre e re-arranca se activo (o universo, o refresh
        # ou o próprio dispositivo podem ter mudado)
        eng.stop_usb_dmx()
        if usb_on:
            if disp is not None:
                ok, err = eng.start_usb_dmx(disp.porta, univ, hz)
                if not ok:
                    messagebox.showerror(
                        T('dmxcfg.title'),
                        T('dmxcfg.usb_start_err', e=err), parent=self)
            # dispositivo «não ligado»: fica configurado, arranca quando
            # a app voltar a encontrá-lo (auto-start)
            if univ not in eng.out_universes():
                # aviso, não bloqueia — o dongle emite zeros (spec)
                messagebox.showwarning(T('dmxcfg.title'),
                                       T('dmxcfg.univ_fora', u=univ),
                                       parent=self)
...
class ChannelPatchDialog(tk.Toplevel):
    ...
    def __init__(self, parent, engine, ch):
        ...
        # v6.4.2 — INVERTER DMX (saída invertida no fio; 16 bit inverte
        # o valor inteiro). Sem isto o R de 1 canal perdia o flag da
        # coluna do renumerador ao gravar.
        self._invert = tk.BooleanVar(value=bool(e.get('invert', False)))
        field(T('cpatch.invert'),
              tk.Checkbutton(f, variable=self._invert, bg='#111122',
...
class MesaDeLuxApp:
    def __init__(self, root):
        self.root = root
        self.root.title("MESADELUX v6.5.0")
        ...
        # v6.4.2 — modo de selecção por atributo: None | 'pan' | 'tilt'
        # (tecla P/T; filtra as selecções da app pela alcunha + nome)
        self._pt_mode = None
        # v6.4.2 — AGARRA/LARGA (catch/drop): {canal: valor congelado}.
        # Agarrados ficam no programador (azuis) e imunes a GO/SEQ/
        # LIBERTA/LIMPA/Comprar até o utilizador LARGAR (fade no tempo
        # do RELEASE). Estado de operação — não vai para o .ldsk.
        self._caught = {}
    ...
    def _auto_start_services(self):
        ...
        # v6.4.1: cada protocolo auto-arranca conforme o toggle guardado
        # na config da máquina (janela «Configuração DMX»)
        try:
            if DMX_SACN_ACTIVO and not self.engine.sacn_enabled and HAS_SACN:
                ...
                self.engine.start_sacn(multicast=self.engine.sacn_multicast,
        ...
        try:
            if DMX_ARTNET_ACTIVO and not self.engine.artnet_sender:
        ...
        # v6.4.1 — USB-DMX: arranca se estava activo e o dispositivo
        # configurado estiver ligado; senão fica «não ligado» (a app
        # arranca normalmente na mesma).
        try:
            if DMX_USB_ACTIVO and HAS_DMX_USB and DMX_USB_DISPOSITIVO:
                self._usb_dmx_arranca_config()
        except Exception as e:
            print('Erro auto-start USB-DMX:', e)
        # vigia do estado USB (apanha o dongle desligado a quente)
        self._usb_est_ant = None
        self.root.after(1000, self._usb_watch)
        # v6.4.2 — tick do AGARRA/LARGA (reafirma os valores agarrados)
        self.root.after(100, self._catch_tick)
    ...
    def _build_ui(self):
        ...
        # v6.4.1b — Idioma e Cuelist (preferências da mesa) vivem no menu
        # Mesa; o resto da antiga janela de Configurações mudou-se para o
        # menu «Configuração» (janelas individuais por protocolo).
        dm.add_command(label=T('menu.lang'),
                       command=lambda: self._open_cfg('lang'))
        dm.add_command(label=T('menu.cuelist'),
                       command=lambda: self._open_cfg('cuelist'))
        dm.add_separator()
        # v6.5 — vista de Layout (página LY do painel esquerdo)
        dm.add_command(label=T('menu.layout'),
                       command=self._menu_layout)
        ...
        # v6.4.1b — menu «Configuração»: uma janela por protocolo/entrada
        cm = tk.Menu(menu, tearoff=0, bg='#1a1a2e', fg='#ccccee')
        menu.add_cascade(label=T('menu.config'), menu=cm)
        cm.add_command(label='sACN…',
                       command=lambda: self._open_cfg('sacn'))
        cm.add_command(label='Art-Net…',
                       command=lambda: self._open_cfg('artnet'))
        cm.add_command(label='OSC…',
                       command=lambda: self._open_cfg('osc'))
        cm.add_command(label='MIDI…',
                       command=lambda: self._open_cfg('midi'))
        cm.add_command(label='DMX-In…',
                       command=lambda: self._open_cfg('dmxin'))
        cm.add_separator()
        cm.add_command(label=T('menu.config_dmx'),
                       command=self._open_config_dmx)
        ...
        # v6.4.1 — estado do USB-DMX (verde activo / cinzento inactivo ou
        # não ligado / vermelho erro)
        self._usb_lbl = tk.Label(status_bar, text="◉ USB OFF",
                                 bg='#111122', fg='#555577', font=('Arial', 9))
        self._usb_lbl.pack(side=tk.RIGHT, padx=8)
    ...
    def _build_channel_panel(self, parent):
        ...
        # v6.4.2 — padx interno 10→8: com as teclas novas (P/T, AGARRA) a
        # fila transbordava e o FX saía do ecrã
        ...
        # v6.4.2 — P/T (selecção PAN/TILT por alcunha), entre o -1 e o
        # B.O. (posição pedida pelo autor)
        self._pt_btn = _blue_btn(ctrl2, "P/T", self._toggle_pt_mode,
                                 width=4, bg='#0f766e')
        self._pt_btn.pack(side=tk.LEFT, padx=2)

        # (v6.4.2: os separadores verticais desta fila SAÍRAM — roubavam
        # ~18 px cada e o FX ficava fora do ecrã; um gap maior chega)
        self._bo_btn = _blue_btn(ctrl2, T('btn.bo'), self._blackout, bg='#4a0000')
        self._bo_btn.pack(side=tk.LEFT, padx=(8, 2))
        _blue_btn(ctrl2, T('btn.release'),
                  self._on_liberta).pack(side=tk.LEFT, padx=2)
        _blue_btn(ctrl2, T('btn.clear'),
                  self._on_clear).pack(side=tk.LEFT, padx=2)
        # v6.4.2 — AGARRA/LARGA (catch/drop): congela os azuis (+ selecção)
        # e larga-os em fade no tempo do RELEASE
        self._catch_btn = _blue_btn(ctrl2, T('btn.catch'),
                                    self._on_catch_drop)
        self._catch_btn.pack(side=tk.LEFT, padx=2)

        # v5 — selecção da página do painel direito (verde = activa)
        self._xf_btn = _blue_btn(ctrl2, "XF",
                                 lambda: self._show_page('xf'), bg='#2c7d4f')
        self._xf_btn.pack(side=tk.LEFT, padx=(8, 2))
        ...
        _hs_btn(">", lambda: self._select_step(+1)).pack(side=tk.LEFT, padx=1)
        # v6.5 — LY: alterna a grelha entre canais e a vista de LAYOUT
        # (páginas de células estilo GrandMA). Verde-azulado; aceso activo.
        self._lay_btn = _hs_btn("LY", self._toggle_lay_view, bg='#2c6e6e')
        self._lay_btn.pack(side=tk.LEFT, padx=(6, 1))
        ...
        self._dmx_btn.pack(side=tk.LEFT, padx=(0, 1))
        ...
        ttk.Label(bar, text=T('dmx.header'),
        ...
        tk.Button(bar, text="▶", command=lambda: self._dmx_step_univ(+1),
                  bg='#33415c', fg='white', relief=tk.FLAT, width=2,
                  cursor='hand2').pack(side=tk.LEFT)
        ttk.Label(bar, text=T('dmx.hint_r'),
        ...
        # ── v6.5: vista LAYOUT (páginas de células estilo GrandMA) ──
        self._lay_view = ttk.Frame(cf)
        self._lay_bar = ttk.Frame(self._lay_view)   # barra de páginas
        self._lay_bar.pack(fill=tk.X, pady=(0, 2))
        self._lay_page_btns = []
        # v6.5 — 10 TECLAS DE ATRIBUTO em baixo (nomeáveis: ZOOM, RED…);
        # TAKE armado + tecla = dar nome; clique = toggle (uma de cada
        # vez). 2026-07-21 (pedido do autor): passaram de 8 para 10 e a
        # barra saiu da vista Layout para o PAINEL dos canais — fica
        # visível em todas as vistas (canais, Layout e DMX).
        self._lay_attr_bar = ttk.Frame(parent)
        self._lay_attr_bar.pack(side=tk.BOTTOM, fill=tk.X, pady=(2, 0))
        self._lay_attr_btns = []
        self._lay_attr_on = None            # índice da tecla activa
        for i in range(10):
            b = tk.Button(self._lay_attr_bar, text='—',
                          command=lambda ii=i: self._lay_attr_click(ii),
                          bg='#2c3e50', fg='#8899aa', relief=tk.FLAT,
                          font=('Arial', 10, 'bold'), pady=4,
                          cursor='hand2')
            b.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
            self._lay_attr_btns.append(b)
        self._lay_canvas = tk.Canvas(self._lay_view, bg='#101018',
                                     highlightthickness=0)
        self._lay_canvas.pack(fill=tk.BOTH, expand=True)
        # v6.5 E2 — selecção: clique = toggle; arrastar = laço tracejado
        # ADITIVO (Esc não se usa: na app é LIBERTA; limpar = LIMPA 1×)
        self._lay_canvas.bind('<Button-1>', self._lay_press)
        self._lay_canvas.bind('<B1-Motion>', self._lay_drag)
        self._lay_canvas.bind('<ButtonRelease-1>', self._lay_release)
        self._lay_canvas.bind('<Button-3>', self._lay_rclick)
        # v6.5 E3b — modo EDIÇÃO gráfica (arrastar células c/ íman de
        # grelha; botão direito tira a célula; nunca junto c/ comando)
        self._lay_edit = False
        self._lay_drag_ch = None
        self._lay_drag_off = (0, 0)
        self._lay_drag_orig = None
        self._lay_attr_map = {}    # célula → canal do atributo activo
        self._lay_items = {}       # ch -> ids dos items de canvas
        self._lay_state = {}       # ch -> tuplo do último estado visual
        self._lay_cells = {}       # ch -> (x, y, w, h) p/ cliques/laço
        self._lay_page = 0         # índice do layout (página) mostrado
        self._lay_src = None       # engine.layouts da última construção
        self._lay_tick_on = False
        self._lay_lasso = None     # id do rectângulo do laço
        self._lay_press_xy = None
        self._lay_press_ch = None
    ...
    def _toggle_dmx_view(self):
        ...
        if self._chan_view == 'lay':     # v6.5: sai do Layout primeiro
            self._lay_leave()
    ...
    def _rb_end(self, event):
        ...
        if sel:
            ...
            self._pt_aplica()             # v6.4.2 — modo P/T filtra
            if len(sel) == 1:
                ch = next(iter(sel))
    ...
    # ── v6.4.2: AGARRA/LARGA (catch/drop) ───────────────────────
    def _exc_protegidos(self):
        """Canais a NÃO libertar/limpar: os do DMX-In + os AGARRADOS
        (catch). None quando não há nenhuns."""
        exc = (self._din_exc() or set()) | set(self._caught)
        return exc or None

    def _on_catch_drop(self):
        """Tecla AGARRA/LARGA. Sem nada agarrado: AGARRA os canais do
        programador (azuis) e os seleccionados (congelados no valor
        actual — ficam azuis). Com canais agarrados: LARGA-os em fade
        com o tempo do RELEASE — voltam ao valor da memória em curso.
        Enquanto agarrados os canais ficam CONGELADOS: GO/SEQ, LIBERTA,
        LIMPA e Comprar/Actualizar não lhes mexem (mas os valores
        agarrados ENTRAM na gravação de cues, como qualquer azul)."""
        if self._caught:
            self._drop_channels()
        else:
            self._catch_channels()

    def _catch_channels(self):
        eng = self.engine
        chs = {ch for ch in range(1, NUM_CHANNELS + 1)
               if eng.programmer[ch] is not None}
        chs |= {ch for ch in self._selected if 1 <= ch <= NUM_CHANNELS}
        if not chs:
            return
        self._capture_undo()
        for ch in sorted(chs):
            v = eng.programmer[ch]
            if v is None:                     # seleccionado sem azul:
                v = eng.output[ch]            # congela no valor ACTUAL
                eng.set_channel(ch, v, fino=True)
            self._caught[ch] = float(v)
        self._update_catch_btn()
        self._refresh()

    def _drop_channels(self):
        """LARGA: só os agarrados saem (fade no tempo do RELEASE); os
        outros azuis do programador ficam como estão."""
        eng = self.engine
        outros = {ch for ch in range(1, NUM_CHANNELS + 1)
                  if eng.programmer[ch] is not None} - set(self._caught)
        exc = (self._din_exc() or set()) | outros
        self._caught = {}
        self._capture_undo()
        eng.clear_programmer(self._release_time, exclude=exc or None)
        self._update_catch_btn()
        self._refresh()

    def _catch_tick(self):
        """Reafirma os valores agarrados (10x/s): o que quer que limpe o
        programador (Comprar, GOTO, consola, …) não desfaz o AGARRA — o
        canal volta ao valor congelado na tick seguinte (mesmo padrão do
        DMX-In). Os caminhos de LIBERTA/LIMPA excluem os agarrados para
        a rampa não lutar com esta tick."""
        if self._caught:
            eng = self.engine
            for ch, v in self._caught.items():
                if 1 <= ch <= NUM_CHANNELS and eng.programmer[ch] != v:
                    b, a = eng._prog_base[ch], eng._prog_abase[ch]
                    eng.set_channel(ch, v, fino=True)
                    if b is not None:
                        # a recaptura não pode corromper o «fundo» — o
                        # valor a que o canal volta no LARGA (o motor
                        # continua a actualizá-lo a cada GO/SEQ via
                        # _update_prog_base)
                        eng._prog_base[ch] = b
                        eng._prog_abase[ch] = a
        self.root.after(100, self._catch_tick)

    def _update_catch_btn(self):
        if not hasattr(self, '_catch_btn'):
            return
        if self._caught:
            self._catch_btn.config(text=T('btn.drop'), bg='#c0392b')
        else:
            self._catch_btn.config(text=T('btn.catch'), bg='#2c3e80')

    def _clear_step2(self):
        """CLEAR 2.º: apaga o programador (azul), com fade.
        v6.4.2: os canais AGARRADOS (catch) ficam."""
        self._capture_undo()
        self.engine.clear_programmer(self._release_time,
                                     exclude=self._exc_protegidos())
        self._refresh()
    ...
    # ── v6.4.2: selecção por atributo PAN/TILT (tecla P/T) ──────
    def _pt_filtra(self, chs):
        """Transforma uma selecção de máquinas na selecção dos seus
        canais PAN ou TILT (conforme o modo): expande pela ALCUNHA ao
        aparelho inteiro (basta ter lá o dimmer) e filtra pelo NOME do
        canal — começa por PAN/TILT, o que inclui PAN16/TILT16 (nomes
        do GDTF e do «Repetir Aparelho»). Máquinas sem alcunha não
        expandem, mas os canais PAN/TILT que JÁ estejam na selecção
        ficam. Sem modo activo devolve a selecção tal e qual."""
        if self._pt_mode not in ('pan', 'tilt') or not chs:
            return set(chs)
        pref = 'PAN' if self._pt_mode == 'pan' else 'TILT'
        fam = self.engine.alias_family(chs)
        return {ch for ch in fam
                if (self.engine.patch.get(ch).get('name') or '')
                .strip().upper().startswith(pref)}

    def _pt_aplica(self):
        """Aplica o filtro do modo P/T à selecção actual (chamado nos
        pontos onde a selecção muda: rectângulo, clique direito,
        expressão, grupos, TODOS). A navegação </> e a selecção vinda
        da consola NÃO passam por aqui (de propósito).
        v6.5: com uma TECLA DE ATRIBUTO do Layout activa (exclusiva com
        o P/T), o filtro é o dela — mesmo mecanismo, nome livre."""
        if self._pt_mode:
            self._selected = self._pt_filtra(self._selected)
        elif getattr(self, '_lay_attr_on', None) is not None:
            self._selected = self._lay_attr_filtra(self._selected)

    def _lay_attr_filtra(self, chs):
        """v6.5 — tecla de atributo activa: expande a selecção pela
        ALCUNHA (como o P/T) e fica com os canais cujo NOME é
        EXACTAMENTE o nome da tecla E são 0-255 (display dec) — regra
        do autor: «o nome bater certo, B = B, ZOOM = ZOOM, nada mais»
        (ao contrário do P/T, que usa prefixo p/ apanhar PAN16)."""
        if getattr(self, '_lay_attr_on', None) is None or not chs:
            return set(chs)
        key = (self.engine.layout_teclas[self._lay_attr_on] or '').strip()
        if not key:
            return set(chs)
        fam = self.engine.alias_family(chs)
        return {ch for ch in fam
                if (self.engine.patch.get(ch).get('name') or '')
                .strip().upper() == key
                and self.engine.patch.get(ch).get('display') == 'dec'}

    def _toggle_pt_mode(self):
        """Tecla P/T: cicla P/T → PAN → TILT → P/T. Ao entrar num modo
        com selecção já feita, transforma-a logo. Ao FECHAR o ciclo
        (voltar a P/T), a selecção converte-se para a INTENSIDADE das
        máquinas — expande pela alcunha e fica com os canais pct (o
        DIM) — em vez de simplesmente desseleccionar (escolha do autor
        2026-07-17, em teste)."""
        self._pt_mode = {None: 'pan', 'pan': 'tilt',
                         'tilt': None}[self._pt_mode]
        # v6.5 — exclusivo com as teclas de atributo do Layout
        if self._pt_mode and getattr(self, '_lay_attr_on', None) is not None:
            self._lay_attr_on = None
            if hasattr(self, '_lay_attr_btns'):
                self._lay_attr_refresh()
        if self._pt_mode and self._selected:
            self._selected = self._pt_filtra(self._selected)
        elif self._pt_mode is None and self._selected:
            fam = self.engine.alias_family(self._selected)
            self._selected = {
                ch for ch in fam
                if self.engine.patch.get(ch).get('display', 'pct')
                == 'pct'}
        self._update_pt_btn()
        self._update_sel_label()
        self._draw_channels()
        # v6.5 — as células do Layout mostram os valores PAN/TILT
        if hasattr(self, '_lay_attr_map'):
            self._lay_attr_map_build()

    def _update_pt_btn(self):
        # cores: repouso TEAL (distinta das outras teclas da fila,
        # pedido do autor); PAN âmbar / TILT roxo (activos)
        if not hasattr(self, '_pt_btn'):
            return
        if self._pt_mode == 'pan':
            self._pt_btn.config(text='PAN', bg='#d68910')
        elif self._pt_mode == 'tilt':
            self._pt_btn.config(text='TILT', bg='#8e44ad')
        else:
            self._pt_btn.config(text='P/T', bg='#0f766e')

    # ── v6.4.2: Consola 1 — encoders PAN/TILT (modo 3 do botão esq) ──
    def _pt_canais(self, atributo):
        """Canais PAN ou TILT da selecção actual, por ordem de canal —
        o mesmo critério da tecla P/T (expande pela alcunha e filtra
        pelo nome; a selecção em si NÃO muda)."""
        pref = 'PAN' if atributo == 'pan' else 'TILT'
        fam = self.engine.alias_family(self._selected)
        return sorted(ch for ch in fam
                      if (self.engine.patch.get(ch).get('name') or '')
                      .strip().upper().startswith(pref))

    def _pt_console_delta(self, atributo, delta):
        """Aplica o delta de um encoder da consola (±5/±3/±1, firmware
        v6) aos canais PAN ou TILT da selecção (via programador, como
        qualquer edição) e devolve o valor novo para o OLED."""
        if atributo not in ('pan', 'tilt') or not delta:
            return
        chs = self._pt_canais(atributo)
        if not chs:
            self._send_console('/pt/%s/value' % atributo, -1)
            return
        self._prog_checkpoint()       # Ctrl+Z volta ao início do gesto
        eng = self.engine
        for ch in chs:
            base = eng.programmer[ch]
            if base is None:
                base = eng.output[ch]
            eng.set_channel(ch, float(base) + delta, fino=True)
        self._pt_console_feedback(atributo)

    def _pt_console_feedback(self, atributo=None):
        """Envia à consola o valor actual (0-255, 1 casa decimal) do
        1.º canal PAN e/ou TILT da selecção; -1 = não há."""
        for atr in (('pan', 'tilt') if atributo is None else (atributo,)):
            chs = self._pt_canais(atr)
            if chs:
                ch = chs[0]
                v = self.engine.programmer[ch]
                if v is None:
                    v = self.engine.output[ch]
                v = round(max(0.0, min(255.0, float(v))), 1)
            else:
                v = -1
            self._send_console('/pt/%s/value' % atr, v)
    ...
    def _update_blackout_btn(self):
        if hasattr(self, '_bo_btn'):
            # mantem SEMPRE o rotulo (B.O.); so muda a cor.
    ...
    # ── v6.5 E1: vista de LAYOUT (página CH ⇄ LY ⇄ DMX, SÓ LEITURA) ──
    # Estilo GrandMA: células (nº/alcunha, nome, valor, halo) e blocos
    # de fundo com nome, a partir da secção «layouts» do show. Vive no
    # painel esquerdo como a grelha e o monitor DMX (tecla LY), com uma
    # barra de PÁGINAS no topo (p.ex. CONVENCIONAL / MÁQUINAS / LEDS).
    # Actualização CIRÚRGICA: guarda os IDs dos itens do Canvas por
    # canal e só toca nas células cujo estado visual mudou; nunca
    # delete('all') no refresh. O desenho corre no ciclo do Tkinter —
    # a saída DMX nunca espera por ele.
    _LAY_W, _LAY_H, _LAY_G, _LAY_O = 62, 52, 4, 24

    def _toggle_lay_view(self):
        """Tecla LY: alterna canais ⇄ Layout. Vindo do DMX, sai do DMX
        primeiro (pelo caminho de sempre, que desliga o teste de saída)."""
        if self._chan_view == 'dmx':
            self._toggle_dmx_view()          # dmx → mesa
        if self._chan_view == 'mesa':
            self._chan_view = 'lay'
            self._mesa_view.pack_forget()
            self._lay_view.pack(fill=tk.BOTH, expand=True)
            self._lay_btn.config(bg='#20b2aa')
            self._layout_build()
            self._lay_tick_start()
        else:                                # lay → mesa
            self._lay_leave()
            self._mesa_view.pack(fill=tk.BOTH, expand=True)
            self._draw_channels()
    ...
    def _lay_leave(self):
        """Sai da vista de Layout (para mesa ou a caminho do DMX).
        Regra do autor: sair desliga a tecla de atributo activa e
        volta-se a funcionar com o sistema dos canais."""
        self._lay_attr_off()
        self._chan_view = 'mesa'
        self._lay_view.pack_forget()
        self._lay_btn.config(bg='#2c6e6e')

    def _lay_attr_off(self):
        """Desliga a tecla de atributo activa (se houver) e devolve a
        selecção à INTENSIDADE (canais pct) das máquinas — usado no
        toggle manual e ao sair da vista de Layout."""
        if getattr(self, '_lay_attr_on', None) is None:
            return
        self._lay_attr_on = None
        if self._selected:
            fam = self.engine.alias_family(self._selected)
    ...
    def _menu_layout(self):
        """Menu Desk → Layout…: vai para a vista (não faz toggle)."""
        if self._chan_view != 'lay':
            self._toggle_lay_view()

    def _lay_set_page(self, i):
        """Tecla de página. COMPRAR armado = JUNTA os canais
        seleccionados a essa página (fluxo do autor: selecciona → TAKE →
        LY → tecla da página); APAGAR armado = apaga a página (com
        confirmação); sem armar, muda de página."""
        if self._record_armed:
            self._record_armed = False
            self._update_comprar_btn()
            self._lay_add_to_page(i)
            return
        if self._delete_armed:
            self._delete_armed = False
            self._update_apagar_btn()
            lays = getattr(self.engine, 'layouts', None) or []
            if 0 <= i < len(lays):
                nome = lays[i].get('nome', '')
                if messagebox.askyesno(T('layout.title'),
                                       T('lay.del_page', n=nome)):
                    self._capture_undo()
                    del lays[i]
                    self._lay_page = 0
                    self._layout_build()
    ...
    def _lay_toggle_edit(self):
        """Botão EDIT: alterna comando ⇄ edição gráfica. Nunca os dois
        ao mesmo tempo (mover uma célula por engano num espectáculo é
        inaceitável — spec)."""
        self._lay_edit = not self._lay_edit
        self._lay_drag_ch = None
        self._layout_build()

    # ── v6.5 E3a: edição por TAKE (páginas e teclas de atributo) ──

    def _lay_uma_por_alias(self, chans):
        """v6.5 — UMA célula por aparelho: canais com alcunha colapsam
        num só por alcunha — de preferência o canal de INTENSIDADE
        (pct, o DIM) do aparelho; senão o mais baixo seleccionado.
        Canais sem alcunha entram todos. (Os atributos não precisam de
        célula: chamam-se com as teclas de atributo.)"""
        out, vistos = [], set()
        for ch in chans:
            e = self.engine.patch.get(ch)
            ...
            if ('a', alc) in vistos:
                continue
            vistos.add(('a', alc))
            fam = self.engine.alias_family({ch})
            pct = sorted(
                c for c in fam
                if self.engine.patch.get(c).get('display', 'pct')
                == 'pct')
            escolhido = pct[0] if pct else ch
            if escolhido not in vistos:
                vistos.add(escolhido)
                out.append(escolhido)
        return out

    def _lay_add_to_page(self, idx):
        """Acrescenta os canais seleccionados à página idx (células
        novas por baixo do que lá está; canais repetidos ignoram-se;
        UMA célula por aparelho/alcunha)."""
        lays = getattr(self.engine, 'layouts', None) or []
        if not (0 <= idx < len(lays)):
            return
        chans = self._lay_uma_por_alias(
            sorted(c for c in self._selected
                   if 1 <= c <= NUM_CHANNELS))
        if not chans:
            messagebox.showinfo(T('layout.title'), T('lay.sem_sel'))
            return
        self._capture_undo()
        page = lays[idx]
        existentes = {c['canal'] for c in page['celulas']}
        novos = [c for c in chans if c not in existentes]
        linha0 = (max((c['linha'] for c in page['celulas']), default=-2)
                  + 2)                       # deixa uma linha de respiro
        for i, ch in enumerate(novos):
            page['celulas'].append({'canal': ch, 'coluna': i % 10,
                                    'linha': linha0 + i // 10})
        self._lay_page = idx
        self._layout_build()

    def _lay_new_page(self):
        """Tecla «+»: com o COMPRAR armado, cria uma página nova com os
        canais seleccionados (pede o nome)."""
        ...
        nome = simpledialog.askstring(T('layout.title'),
                                      T('lay.page_name'),
                                      parent=self.root)
        if nome is None:
            return
        self._capture_undo()
        lays = getattr(self.engine, 'layouts', None)
        if lays is None:
            lays = self.engine.layouts = []
        lid = max((l.get('id', 0) for l in lays), default=0) + 1
        cels = [{'canal': ch, 'coluna': i % 10, 'linha': i // 10}
                for i, ch in enumerate(chans)]
        lays.append({'id': lid,
                     'nome': (nome.strip() or f'LAYOUT {lid}')[:20],
                     'celulas': cels, 'blocos': []})
        self._lay_page = len(lays) - 1
        self._layout_build()

    def _lay_attr_click(self, i):
        """Tecla de atributo (barra de baixo). COMPRAR armado = dar
        nome (fluxo do autor: TAKE → tecla → nome → OK); APAGAR armado
        = limpar o nome; sem armar = toggle (uma activa de cada vez),
        com a semântica do P/T: ao ACTIVAR, a selecção expande pela
        alcunha e filtra aos canais com esse NOME e display 0-255,
        mostrando o valor deles; ao DESACTIVAR volta à intensidade
        (canais pct) das máquinas."""
        if self._delete_armed:              # APAGAR + tecla = limpa nome
            self._delete_armed = False
            self._update_apagar_btn()
            if self.engine.layout_teclas[i]:
                self._capture_undo()
                self.engine.layout_teclas[i] = ''
                if self._lay_attr_on == i:
                    self._lay_attr_on = None
                self._lay_attr_map_build()
                self._lay_attr_refresh()
            return
        if self._record_armed:
            ...
            self._update_comprar_btn()
            from tkinter import simpledialog
            actual = self.engine.layout_teclas[i]
            nome = simpledialog.askstring(T('layout.title'),
                                          T('lay.key_name'),
                                          initialvalue=actual,
                                          parent=self.root)
            if nome is None:
                return
            self._capture_undo()
            self.engine.layout_teclas[i] = nome.strip().upper()[:8]
            if self._lay_attr_on == i and not self.engine.layout_teclas[i]:
                self._lay_attr_on = None
            self._lay_attr_map_build()
            self._lay_attr_refresh()
            return
        if not self.engine.layout_teclas[i]:
            return                          # tecla em branco não activa
        if self._lay_attr_on == i:          # desactivar → intensidade
            self._lay_attr_off()
        else:                               # activar → filtra e mostra
            self._lay_attr_on = i
            if self._pt_mode:               # exclusivo com o P/T
                self._pt_mode = None
                self._update_pt_btn()
            if self._selected:
                self._selected = self._lay_attr_filtra(self._selected)
                chs = sorted(self._selected)
                if chs:                     # o valor do atributo aparece
                    self._level_var.set(
                        str(int(round(self.engine.output[chs[0]]))))
        self._lay_attr_map_build()
        self._lay_attr_refresh()
        self._update_sel_label()
        self._draw_channels()

    def _lay_attr_refresh(self):
        """Nomes e estado visual das 10 teclas de atributo."""
        teclas = getattr(self.engine, 'layout_teclas', None) or [''] * 10
        for i, b in enumerate(self._lay_attr_btns):
            nome = teclas[i] if i < len(teclas) else ''
            if self._lay_attr_on == i and nome:
                b.config(text=nome, bg='#e8d44d', fg='#1a1a1a')
            elif nome:
                b.config(text=nome, bg='#2c6e6e', fg='white')
            else:
                b.config(text='—', bg='#2c3e50', fg='#8899aa')

    def _lay_attr_map_build(self):
        """v6.5 — mapa célula→canal do atributo da tecla ACTIVA, pela
        alcunha: com p.ex. ZOOM carregado, cada célula do layout passa
        a mostrar o valor do ZOOM do SEU aparelho (pedido do autor:
        «devia aparecer em todas as células que têm um valor naquele
        nome»). Vazio quando não há tecla activa."""
        self._lay_attr_map = {}
        i = getattr(self, '_lay_attr_on', None)
        key, exacto = None, True
        if i is not None:
            key = (self.engine.layout_teclas[i] or '').strip()
        elif self._pt_mode in ('pan', 'tilt'):
            # v6.5 — o P/T também mostra os valores nas células (pedido
            # do autor), com a regra de PREFIXO dele (PAN apanha PAN16)
            key = 'PAN' if self._pt_mode == 'pan' else 'TILT'
            exacto = False
        if not key:
            return
        por_alias = {}
        for ch in range(1, NUM_CHANNELS + 1):
            e = self.engine.patch.get(ch)
            alc = int(e.get('alcunha') or 0)
            nome = (e.get('name') or '').strip().upper()
            bate = (nome == key) if exacto else nome.startswith(key)
            if (alc and alc not in por_alias
                    and e.get('display') == 'dec' and bate):
                por_alias[alc] = ch
        for ch in self._lay_items:
            alc = int(self.engine.patch.get(ch).get('alcunha') or 0)
            ach = por_alias.get(alc)
            if ach:
                self._lay_attr_map[ch] = ach

    def _lay_attr_valor(self):
        """v6.5 — com uma tecla activa, o VALOR do atributo aparece na
        própria tecla (ex.: «ZOOM 127»; vários canais com valores
        diferentes → mostra o primeiro e «…»). Chamado pelo tick."""
        i = self._lay_attr_on
        if i is None:
            return
        nome = self.engine.layout_teclas[i]
        if not nome:
            return
        b = self._lay_attr_btns[i]
        chs = sorted(self._selected)
        if not chs:
            txt = nome
        else:
            vals = {int(round(self.engine.output[c])) for c in chs}
            v = int(round(self.engine.output[chs[0]]))
            txt = f'{nome} {v}' + ('…' if len(vals) > 1 else '')
        if b.cget('text') != txt:
            b.config(text=txt)

    # ── v6.5 E2: selecção na vista de Layout ─────────────────────
    # Mesma selecção central da app (self._selected): o que se marca
    # aqui acende na grelha, na expressão escrita e na consola, e
    # vice-versa. Clique numa célula = selecciona/desselecciona; laço
    # tracejado (arrastar em zona vazia ou sobre células) = ADITIVO,
    # como o arrasto da grelha. O modo P/T filtra como em todo o lado.

    def _lay_ch_at(self, x, y):
        for ch, (cx, cy, cw, chh) in self._lay_cells.items():
            if cx <= x <= cx + cw and cy <= y <= cy + chh:
                return ch
        return None

    def _lay_press(self, event):
        if self._lay_edit:                   # modo EDIÇÃO: pega na célula
            ch = self._lay_ch_at(event.x, event.y)
            self._lay_drag_ch = ch
            if ch is not None:
                x, y, _w, _h = self._lay_cells[ch]
                self._lay_drag_off = (event.x - x, event.y - y)
                W, H, G, O = (self._LAY_W, self._LAY_H,
                              self._LAY_G, self._LAY_O)
                self._lay_drag_orig = (round((x - O) / (W + G)),
                                       round((y - O - 18) / (H + G)))
            return
        self._lay_press_xy = (event.x, event.y)
        self._lay_press_ch = self._lay_ch_at(event.x, event.y)

    def _lay_drag(self, event):
        if self._lay_edit:                   # arrasta c/ íman de grelha
            ch = self._lay_drag_ch
            if ch is None:
                return
            W, H, G, O = (self._LAY_W, self._LAY_H,
                          self._LAY_G, self._LAY_O)
            col = max(0, round((event.x - self._lay_drag_off[0] - O)
                               / (W + G)))
            lin = max(0, round((event.y - self._lay_drag_off[1] - O - 18)
                               / (H + G)))
            nx = O + col * (W + G)
            ny = O + 18 + lin * (H + G)
            ox, oy, w, h = self._lay_cells[ch]
            if (nx, ny) != (ox, oy):
                dx, dy = nx - ox, ny - oy
                for item in self._lay_items[ch]:
                    self._lay_canvas.move(item, dx, dy)
                self._lay_cells[ch] = (nx, ny, w, h)
            return
        if self._lay_press_xy is None:
            return
        x0, y0 = self._lay_press_xy
        if self._lay_lasso is None:
            if abs(event.x - x0) < 5 and abs(event.y - y0) < 5:
                return                       # ainda conta como clique
            self._lay_lasso = self._lay_canvas.create_rectangle(
                x0, y0, event.x, event.y, outline='#ffaa00',
                dash=(4, 3), width=1)
        else:
            self._lay_canvas.coords(self._lay_lasso, x0, y0,
                                    event.x, event.y)

    def _lay_release(self, event):
        if self._lay_edit:                   # larga: grava a posição nova
            ch, self._lay_drag_ch = self._lay_drag_ch, None
            if ch is None:
                return
            lay = self._layout_actual()
            if not lay:
                return
            W, H, G, O = (self._LAY_W, self._LAY_H,
                          self._LAY_G, self._LAY_O)
            x, y, _w, _h = self._lay_cells[ch]
            col = max(0, round((x - O) / (W + G)))
            lin = max(0, round((y - O - 18) / (H + G)))
            if (col, lin) == self._lay_drag_orig:
                return                       # não se mexeu — sem undo
            self._capture_undo()
            for cel in lay['celulas']:
                if cel['canal'] == ch:
                    cel['coluna'], cel['linha'] = col, lin
                    break
            self._layout_build()             # blocos redesenham à volta
            return
        if self._lay_press_xy is None:
            return
        x0, y0 = self._lay_press_xy
        self._lay_press_xy = None
        if self._lay_lasso is not None:      # laço → selecção aditiva
            self._lay_canvas.delete(self._lay_lasso)
            self._lay_lasso = None
            rx0, rx1 = min(x0, event.x), max(x0, event.x)
            ry0, ry1 = min(y0, event.y), max(y0, event.y)
            sel = {ch for ch, (cx, cy, cw, chh) in self._lay_cells.items()
                   if cx <= rx1 and cx + cw >= rx0
                   and cy <= ry1 and cy + chh >= ry0}
            if not sel:
                return
            # como na grelha: depois de mexer num nível, a selecção
            # seguinte LIMPA a anterior (e desbloqueia)
            if self._sel_locked:
                self._selected.clear()
                self._sel_locked = False
            self._selected |= sel
            if len(sel) == 1:
                ch = next(iter(sel))
                self._level_var.set(str(int(round(self.engine.output[ch]))))
        else:                                # clique → toggle do canal
            ch, self._lay_press_ch = self._lay_press_ch, None
            if ch is None:
                return
            if self._sel_locked:             # mexeu num nível → substitui
                self._selected.clear()
                self._sel_locked = False
                self._selected.add(ch)
                self._level_var.set(str(int(round(self.engine.output[ch]))))
            elif ch in self._selected:
                self._selected.discard(ch)
            else:
                self._selected.add(ch)
                self._level_var.set(str(int(round(self.engine.output[ch]))))
        self._step_group = None              # selecção manual sai do grupo
        self._pt_aplica()                    # v6.4.2 — modo P/T filtra
        self._update_sel_label()
        self._sync_console_selection()       # a consola segue (como a grelha)

    def _lay_rclick(self, event):
        """Botão direito no modo EDIÇÃO: tira a célula da página (o
        canal continua no show; Ctrl+Z desfaz). Fora do modo, nada."""
        if not self._lay_edit:
            return
        ch = self._lay_ch_at(event.x, event.y)
        lay = self._layout_actual()
        if ch is None or not lay:
            return
        self._capture_undo()
        lay['celulas'] = [c for c in lay['celulas'] if c['canal'] != ch]
        for bl in lay.get('blocos', []):
            bl['celulas'] = [c for c in bl['celulas'] if c != ch]
        self._layout_build()

    def _layout_actual(self):
        """O layout (página) mostrado, ou None."""
        lays = getattr(self.engine, 'layouts', None) or []
        if not lays:
            return None
        if not (0 <= self._lay_page < len(lays)):
            self._lay_page = 0
        return lays[self._lay_page]

    def _layout_build(self):
        """(Re)constrói a barra de páginas e os itens do Canvas. Corre
        ao entrar na vista, ao mudar de página e quando o show muda —
        NUNCA no refresh normal."""
        cv = self._lay_canvas
        cv.delete('all')
        self._lay_items = {}
        self._lay_state = {}
        self._lay_cells = {}
        self._lay_lasso = None
        # assinatura = a própria lista do engine (o load/new substitui o
        # objecto; comparação por identidade é barata e chega na E1)
        self._lay_src = getattr(self.engine, 'layouts', None)
        lays = self._lay_src or []
        # barra de páginas (uma tecla por layout; activa em destaque)
        for b in self._lay_page_btns:
            b.destroy()
        self._lay_page_btns = []
        lay = self._layout_actual()
        for i, l in enumerate(lays):
            b = tk.Button(self._lay_bar,
                          text=(l.get('nome') or str(i + 1)),
                          command=lambda ii=i: self._lay_set_page(ii),
                          bg='#20b2aa' if i == self._lay_page else '#2c3e50',
                          fg='white', relief=tk.FLAT, padx=8, pady=2,
                          font=('Arial', 9, 'bold'), cursor='hand2')
            b.pack(side=tk.LEFT, padx=2)
            self._lay_page_btns.append(b)
        # «+»: com o COMPRAR armado cria uma página nova com a selecção
        b = tk.Button(self._lay_bar, text='+', command=self._lay_new_page,
                      bg='#3a5068', fg='white', relief=tk.FLAT, padx=8,
                      pady=2, font=('Arial', 9, 'bold'), cursor='hand2')
        b.pack(side=tk.LEFT, padx=(8, 2))
        self._lay_page_btns.append(b)
        # EDIT: modo de edição gráfica (vermelho quando activo)
        b = tk.Button(self._lay_bar, text='EDIT',
                      command=self._lay_toggle_edit,
                      bg='#c0392b' if self._lay_edit else '#3a5068',
                      fg='white', relief=tk.FLAT, padx=8, pady=2,
                      font=('Arial', 9, 'bold'), cursor='hand2')
        ...
        self._lay_attr_refresh()
        if not lay:
            cv.create_text(20, 20, anchor='nw', fill='#8888aa',
                           font=('Consolas', 12), text=T('layout.none'))
            return
        W, H, G, O = self._LAY_W, self._LAY_H, self._LAY_G, self._LAY_O

        # modo EDIT: grelha de alinhamento por trás de tudo
        if self._lay_edit:
            wpx = max(cv.winfo_width(), 1100)
            hpx = max(cv.winfo_height(), 700)
            for gx in range(O - G // 2, wpx, W + G):
                cv.create_line(gx, 0, gx, hpx, fill='#1e1e32')
            for gy in range(O + 18 - G // 2, hpx, H + G):
                cv.create_line(0, gy, wpx, gy, fill='#1e1e32')

        pos = {}
        for cel in lay.get('celulas', []):
            pos[cel['canal']] = (O + cel['coluna'] * (W + G),
                                 O + 18 + cel['linha'] * (H + G))

        # blocos de fundo (ficam atrás das células)
        for bl in lay.get('blocos', []):
            xs = [pos[c] for c in bl.get('celulas', []) if c in pos]
            if not xs:
                continue
            x1 = min(p[0] for p in xs) - 5
            y1 = min(p[1] for p in xs) - 5
            x2 = max(p[0] for p in xs) + W + 5
            y2 = max(p[1] for p in xs) + H + 5
            cv.create_rectangle(x1, y1 - 13, x2, y2, fill='#181826',
                                outline='#2c2c3e')
            cv.create_text(x1 + 3, y1 - 4, anchor='sw',
                           text=bl.get('nome', ''), fill='#8888aa',
                           font=('Consolas', 9, 'bold'))

        cv.create_text(O, 6, anchor='nw', text=lay.get('nome', ''),
                       fill='#c8c8e8', font=('Consolas', 11, 'bold'))

        for cel in lay.get('celulas', []):
            ch = cel['canal']
            x, y = pos[ch]
            existe = 1 <= ch <= NUM_CHANNELS
            halo_id = cv.create_rectangle(x - 2, y - 2, x + W + 2,
                                          y + H + 2, outline='', width=3)
            rect_id = cv.create_rectangle(
                x, y, x + W, y + H, fill='#16162a',
                outline='#2c2c3e' if existe else '#3a3a3a', width=1)
            num_id = cv.create_text(x + 4, y + 3, anchor='nw', text='',
                                    fill='#f0d840',
                                    font=('Consolas', 10, 'bold'))
            name_id = cv.create_text(x + W - 4, y + 3, anchor='ne',
                                     text='', fill='#8888aa',
                                     font=('Consolas', 8))
            val_id = cv.create_text(x + W / 2, y + H - 12, text='',
                                    fill='#c8c800',
                                    font=('Consolas', 11, 'bold'))
            if existe:
                self._lay_items[ch] = (halo_id, rect_id, num_id,
                                       name_id, val_id)
                self._lay_cells[ch] = (x, y, W, H)   # E2: área clicável
            else:
                # canal fora do show: cinzento com aviso, nunca erro
                cv.itemconfig(num_id, text=str(ch), fill='#555555')
                cv.itemconfig(val_id, text='?', fill='#555555')
        self._lay_attr_map_build()

    def _lay_tick_start(self):
        if not self._lay_tick_on:
            self._lay_tick_on = True
            self._layout_tick()

    def _layout_tick(self):
        """Refresh cirúrgico ~25 Hz enquanto a vista LY está activa."""
        if self._chan_view != 'lay':
            self._lay_tick_on = False
            return
        eng = self.engine
        if getattr(eng, 'layouts', None) is not getattr(
                self, '_lay_src', None):
            self._layout_build()          # abriu-se outro show
        cv = self._lay_canvas
        fxm = eng.fx_levels
        for ch, (halo_id, rect_id, num_id, name_id, val_id) \
                in self._lay_items.items():
            e = eng.patch.get(ch)
            play = eng.output[ch]
            prog = eng.programmer[ch] is not None
            fxv = fxm.get(ch) if (fxm and not prog) else None
            base, fx_dom = play, False
            if fxv is not None and fxv > base:
                base, fx_dom = fxv, True
            sub = eng.submaster_contribution(ch)
            lvl_i = int(round(max(base, sub)))
            sub_dom = sub > base and sub > 0
            if sub_dom:
                fx_dom = False
            selected = ch in self._selected
            passive = not eng.active[ch]
            halo = e.get('halo')
            name = (e.get('name') or '')[:6]
            alc = e.get('alcunha') or 0
            display = e.get('display')
            # v6.5 — tecla de atributo activa: a célula mostra o valor
            # DESSE atributo do seu aparelho (amarelo, como a tecla)
            ach = self._lay_attr_map.get(ch)
            att_v = int(round(eng.output[ach])) if ach else None
            state = (lvl_i, prog, selected, passive, sub_dom, fx_dom,
                     halo, name, alc, display, att_v)
            if self._lay_state.get(ch) == state:
                continue                  # célula sem alterações
            self._lay_state[ch] = state
            # cores — mesmas famílias da grelha principal
            if prog:
                fill, outline = '#1a3a5c', '#5dade2'
            elif fx_dom:
                fill, outline = '#3a2a10', '#e67e22'
            elif sub_dom:
                fill, outline = '#241038', '#9b6dd6'
            elif not passive:
                g = int(40 + lvl_i / 255 * 120)
                fill, outline = f'#00{g:02x}30', '#2ecc71'
            else:
                fill, outline = '#16162a', '#2c2c3e'
            cv.itemconfig(rect_id, fill=fill,
                          outline='#e8d44d' if selected else outline,
                          width=2 if selected else 1)
            cv.itemconfig(halo_id,
                          outline=HALO_COLORS.get(halo) or '')
            cv.itemconfig(num_id, text=str(alc) if alc else str(ch),
                          fill='#ff9ec9' if alc else '#f0d840')
            cv.itemconfig(name_id, text=name)
            if att_v is not None:         # valor do ATRIBUTO da tecla
                cv.itemconfig(val_id, text=str(att_v), fill='#e8d44d')
            else:
                if display == 'dec':
                    txt = str(lvl_i) if lvl_i > 0 else '—'
                else:
                    pct = round(lvl_i / 255 * 100)
                    txt = f'{pct}%' if pct > 0 else '—'
                cv.itemconfig(val_id, text=txt,
                              fill='#ffffff' if prog else '#c8c800')
        self._lay_attr_valor()      # valor do atributo na tecla activa
        self.root.after(40, self._layout_tick)
    ...
    def _refresh_status(self):
        ...
        # v6.4.1 — USB-DMX
        u_est, _u_err = self.engine.usb_estado()
        if u_est == 'activo':
            self._usb_lbl.config(
                text="◉ USB  U%d@%dHz" % (self.engine.usb_universo,
                                          self.engine.usb_refresh),
                fg='#2ecc71')
        elif u_est == 'erro':
            self._usb_lbl.config(text="◉ USB ERRO", fg='#e74c3c')
        elif DMX_USB_ACTIVO:
            # configurado mas sem dispositivo ligado
            self._usb_lbl.config(text="◉ USB —", fg='#555577')
        else:
            self._usb_lbl.config(text="◉ USB OFF", fg='#555577')
    ...
    def apply_midi_ports(self, in_name, out_name):
        ...
        as portas. Não reinicia a aplicação.
        v6.4.2c — só mexe na porta que MUDOU: fechar e reabrir a mesma
        porta WinMM sem necessidade era mais uma ocasião para o driver
        encravar (e cortava a escuta sem razão)."""
        global MIDI_IN_PORT, MIDI_OUT_PORT
        in_mudou = (in_name != self.midi_in_name
                    or (in_name and self._midi_in is None))
        out_mudou = (out_name != self.midi_out_name
                     or (out_name and self._midi_out is None))
        ...
        save_app_config()
        if out_mudou:
            self._open_midi_out()          # fecha a antiga e abre a nova
        if in_mudou:
            self.start_midi_in()           # idem (None = só fecha)
        self._refresh_status()
    ...
    def _close_midi_in(self):
        if self._midi_in is not None:
            # v6.4.2c — SOLTA o callback ANTES de fechar: fechar uma
            # porta IN do WinMM com o callback ainda armado é a causa
            # conhecida do «python bloqueia às vezes» no Windows
            # (deadlock no driver rtmidi durante o close).
            try:
                self._midi_in.callback = None
            except Exception:
                pass
            time.sleep(0.05)      # deixa terminar um callback em curso
            try:
                self._midi_in.close()
            except Exception:
                pass
    ...
    def _osc_apply(self, addr, args):
        ...
        # v6.4.2 — Consola 1 (firmware v6), modo PAN/TILT (3.º modo do
        # botão esquerdo): os encoders mandam deltas ±1 que se aplicam
        # aos canais PAN/TILT da selecção actual (alcunha + nome, o
        # mesmo critério da tecla P/T). '/pt/mode 1' pede os valores
        # actuais para os OLEDs; a cada delta devolve-se o valor novo.
        if addr == '/pt/mode' and args:
            if bool(int(float(args[0]))):
                self._pt_console_feedback()
            return
        if addr in ('/pt/pan', '/pt/tilt') and args:
            # passos da consola: ±5 / ±3 / ±1 (aceita-se float por
            # robustez do transporte)
            self._pt_console_delta(addr[4:], float(args[0]))
            return
    ...
    def _console_release_all(self, duration_s):
        ...
        exc = self._exc_protegidos() or set()   # v6.4.2: + agarrados
        start_levels = {}
        for ch in range(1, NUM_CHANNELS + 1):
    ...
    def _new_show(self):
        ...
        self._selected = set()
        self._caught = {}                 # v6.4.2: show novo larga tudo
        self._update_catch_btn()
    ...
    def _open_show(self):
        ...
        if p:
            try:
                ...
                self._caught = {}         # v6.4.2: mudar de show larga tudo
                self._update_catch_btn()
    ...
    # ── USB-DMX (v6.4.1) ──────────────────────
    def _open_config_dmx(self):
        ConfigDMXDialog(self)

    # v6.4.1b — abre UMA secção de configuração como janela própria
    # (menu «Configuração», e Idioma/Cuelist a partir do menu Mesa)
    def _open_cfg(self, key):
        SettingsDialog(self.root, self, only=key)

    def _usb_dmx_arranca_config(self):
        """Arranca o USB-DMX a partir da configuração guardada, se o
        dispositivo estiver ligado. Devolve True se arrancou. Se a COM
        guardada mudou mas há exactamente um adaptador FTDI ligado
        (dispositivo único por desenho), usa esse e actualiza a config
        (nota no terminal)."""
        global DMX_USB_DISPOSITIVO
        cfg = DMX_USB_DISPOSITIVO
        if not (HAS_DMX_USB and cfg):
            return False
        d, nota = dmx_usb.encontrar_dispositivo(
            cfg.get('id'), cfg.get('tipo_id'), cfg.get('porta_com'))
        if d is None:
            print('[USB-DMX] dispositivo configurado não ligado (%s).'
                  % (cfg.get('porta_com') or cfg.get('id')))
            self._refresh_status()
            return False
        if nota:
            print('[USB-DMX]', nota)
            cfg = dict(cfg)
            cfg['porta_com'] = d.porta
            if cfg.get('tipo_id') == 'porta':
                cfg['id'] = d.porta
            DMX_USB_DISPOSITIVO = cfg
            save_app_config()
        elif cfg.get('porta_com') != d.porta:
            # mesmo nº de série noutra COM — actualiza silenciosamente
            cfg = dict(cfg)
            cfg['porta_com'] = d.porta
            DMX_USB_DISPOSITIVO = cfg
            save_app_config()
        ok, err = self.engine.start_usb_dmx(d.porta,
                                            cfg.get('universo', 1),
                                            cfg.get('refresh_hz', 30))
        if not ok:
            print('[USB-DMX] erro ao arrancar:', err)
        self._refresh_status()
        return ok

    def _usb_watch(self):
        """v6.4.1 — vigia o estado da thread USB-DMX de 1 em 1 s: se o
        dongle for desligado a quente, a thread morre com estado 'erro'
        e o indicador da barra tem de o mostrar sem depender de nenhum
        outro refresh."""
        est, _ = self.engine.usb_estado()
        if est != self._usb_est_ant:
            self._usb_est_ant = est
            self._refresh_status()
        self.root.after(1000, self._usb_watch)
