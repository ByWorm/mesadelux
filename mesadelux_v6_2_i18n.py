...
def save_app_config():
    """Grava as preferências do utilizador (idioma, …)."""
    try:
...
class Engine:
    ...
    def _arm_follow(self):
        """Após terminar o fade, agenda a memória seguinte se esta tiver follow."""
        if 0 <= self.current_cue_idx < len(self.cues):
            fol = self.cues[self.current_cue_idx].get('follow')
            if fol is not None and fol >= 0 and self.next_index(self.current_cue_idx) is not None:
...
class RepetirFixtureDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        self.v_alc  = field(4, T('rep.f_alc'), '')   # vazia = sem alcunha
...
class MesaDeLuxApp:
    def __init__(self, root):
        self.root = root
        self.root.title("MESADELUX v6.2")
    ...
    def _pump(self):
        ...
        try:
            ...
            pass
        if self._dirty or self.engine.fading:
            self._dirty = False
            self._refresh()
    ...
    def _update_grid(self):
        ...
        for ch, (halo_id, rect_id, bar_id, name_id, num_id, val_id, x, y) \
            ...
            # nome (topo) + número do canal OU alcunha (centro, amarelo)
    ...
    def _update_fade_bar(self):
        """Mostra o progresso da transição lançada pelo VAI."""
        p = self.engine.fade_progress
        self._fade_bar['value'] = p * 100
        if self.engine.fading:
            self._fade_lbl.config(text=f"{int(round(p * 100))}%",
                                  foreground='#e8d44d')
