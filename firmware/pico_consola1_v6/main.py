# =============================================================================
# Mesadeluz-OSC_CONSOLA — Consola 1 (firmware Pico W)  -  v6
# =============================================================================
# v6 vs v5 (modo PAN/TILT — para a app MesaDeLux v6.4.2+):
#   - O botao ESQ passa a CICLAR 3 modos: CANAL -> CUELIST -> PAN/TILT ->
#     CANAL (1 clique avanca; do PAN/TILT um clique volta a CANAL).
#     O duplo clique (LIBERTA TUDO) mantem-se.
#   - MODO PAN/TILT: OLED esq mostra PAN, OLED dir mostra TILT. O encoder
#     ESQ manda '/pt/pan' e o DIR '/pt/tilt' com o delta — o botao DIR
#     cicla o passo 5 -> 3 -> 1 (0-255, inteiros; os meios-pontos foram
#     RETIRADOS a pedido do autor 2026-07-18). A app aplica
#     aos canais PAN/TILT (alcunha + nome) da seleccao actual e devolve
#     '/pt/pan/value' / '/pt/tilt/value' (valor 0-255 do 1.o canal;
#     -1 = nao ha nenhum na seleccao -> OLED mostra '---').
#     Ao entrar envia '/pt/mode 1' (a app responde com os valores);
#     ao sair '/pt/mode 0'.
#
...
#   MODE_PANTILT (v6)       — encoders = PAN (esq) / TILT (dir) da seleccao
#
# Botao ESQ: 1x => cicla CHANNEL -> CUELIST -> PANTILT -> CHANNEL
#            2x => LIBERTA TUDO (fade 2 s)   (gap >400 ms p/ duplo clique)
#
# CHANNEL:
...
MODE_PANTILT = const(2)      # v6: encoders = PAN/TILT da seleccao
...
app_univ          = 0
dmx_browse        = 1
# v6: valores PAN/TILT do modo PANTILT (0-255; -1 = a seleccao nao tem
# nenhum canal desse atributo -> OLED mostra '---'). A app envia-os via
# '/pt/pan/value' e '/pt/tilt/value'; a consola tambem os ajusta
# localmente a cada detente (a resposta da app corrige, se preciso).
pt_pan_val        = -1
pt_tilt_val       = -1
# v6: passo dos encoders no modo PANTILT (botao DIR cicla 5 -> 3 -> 1;
# inteiros — os meios-pontos foram retirados a pedido do autor)
PT_STEPS          = (5, 3, 1)
pt_step_idx       = 0
...
# ---------------------------------------------------------------
def pt_fmt(v):
    """v6: valor PAN/TILT p/ OLED — '---' sem canal na seleccao; inteiro
    quando certo, senao 1 casa decimal (meios-passos do 0.5/2.5)."""
    if v < 0:
        return '---'
    if abs(v - int(v)) < 0.05:
        return str(int(v))
...
def update_oled1():
    ...
    elif mode == MODE_PANTILT:
        # v6: OLED esq = PAN + valor (0-255) do 1.o canal PAN da seleccao
        text2x(oled1, 'PAN', cx2('PAN'), 0)
        v_str = pt_fmt(pt_pan_val)
        text2x(oled1, v_str, cx2(v_str), 26)
        st = '+/-{}'.format(PT_STEPS[pt_step_idx])
        oled1.text(st, cx1(st), 46)
    else:   # MODE_CUELIST
...
def update_oled2():
    ...
    if mode == MODE_CHANNEL:
        ...
        # v6: OLED dir = TILT + valor (0-255) do 1.o canal TILT da seleccao
        text2x(oled2, 'TILT', cx2('TILT'), 0)
        v_str = pt_fmt(pt_tilt_val)
        text2x(oled2, v_str, cx2(v_str), 26)
        st = '+/-{}'.format(PT_STEPS[pt_step_idx])
        oled2.text(st, cx1(st), 46)
...
def handle_osc_msg(address, args):
    ...
    global pt_pan_val, pt_tilt_val

    # v6: valores PAN/TILT devolvidos pela app (modo PANTILT; float —
    # os meios-passos do 0.5/2.5 chegam com 1 casa decimal)
    if address == '/pt/pan/value' and args:
        try:    pt_pan_val = float(args[0])
        except: pt_pan_val = -1
        if mode == MODE_PANTILT:
            req_draw(o1=True)
        return
    if address == '/pt/tilt/value' and args:
        try:    pt_tilt_val = float(args[0])
        except: pt_tilt_val = -1
        if mode == MODE_PANTILT:
            req_draw(o2=True)
        return
...
def cycle_pt_step():
    # v6: passo dos encoders PAN/TILT — 5 -> 3 -> 1 -> 5
    global pt_step_idx
    pt_step_idx = (pt_step_idx + 1) % len(PT_STEPS)
    req_draw(o1=True, o2=True)
...
def toggle_mode():
    # v6: cicla 3 modos — CANAL -> CUELIST -> PAN/TILT -> CANAL
    global mode
    if highlight_mode:                 # v6.2: sai do Highlight ao trocar de modo
        toggle_highlight_mode()
    if mode == MODE_PANTILT:
        send_osc_i('/pt/mode', 0)      # avisa a app que saimos
    mode = (mode + 1) % 3
    if mode == MODE_CUELIST:
        send_osc_empty('/cue/state/request')
    elif mode == MODE_PANTILT:
        send_osc_i('/pt/mode', 1)      # a app responde com /pt/*/value
    update_display()
...
while True:
    ...
    if cur_esq != last_esq:
        ...
        if mode == MODE_CHANNEL:
            ...
            # v6: encoder ESQ = PAN, delta = passo actual (inteiro)
            delta = d * PT_STEPS[pt_step_idx]
            send_osc_i('/pt/pan', delta)
            if pt_pan_val >= 0:            # ajuste local p/ OLED reactivo
                pt_pan_val = max(0, min(255, pt_pan_val + delta))
                req_draw(o1=True)
        else:    # MODE_CUELIST
            if d > 0: do_cue_go()
            else:     do_cue_back()
    ...
    if sw_dir_p and not _sw_dir_prev:
        ...
        if not _sw_dir_long and held > 60:   # largou cedo -> accao CURTA
            ...
            elif mode == MODE_CUELIST: toggle_cue_edit()
            elif mode == MODE_PANTILT: cycle_pt_step()   # 2.5 -> 1 -> 0.5
    ...
    if cur_dir != last_dir:
        ...
        if mode == MODE_CHANNEL:
            ...
            # v6: encoder DIR = TILT, delta = passo actual (inteiro)
            delta = d_sign * PT_STEPS[pt_step_idx]
            send_osc_i('/pt/tilt', delta)
            if pt_tilt_val >= 0:           # ajuste local p/ OLED reactivo
                pt_tilt_val = max(0, min(255, pt_tilt_val + delta))
                req_draw(o2=True)
        else:
